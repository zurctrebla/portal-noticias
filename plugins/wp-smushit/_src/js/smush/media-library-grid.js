...
(function ($, _) {
	...
	const prepareTemplate = function (smushHTML) {
		/**
		 * @param {Array}  smush_global.strings  Localization strings.
		 * @param {Object} smush_global          Object from wp_localize_script()
		 */
		return template({
			label: smush_global.strings.stats_label,
			value: smushHTML,
		});
	};
	...
	const MediaLibraryTaxonomyFilter = wp.media.view.AttachmentFilters.extend({
		...
		createFilters() {
			this.filters = {
				all: {
					text: smush_global.strings.filter_all,
					props: { stats: 'all' },
					priority: 10,
				},

				unsmushed: {
					text: smush_global.strings.filter_not_processed,
				...
				failed: {
					text: smush_global.strings.filter_failed,
