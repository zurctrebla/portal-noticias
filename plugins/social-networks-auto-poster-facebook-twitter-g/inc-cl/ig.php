...
if (!class_exists("nxs_snapClassIG")) { class nxs_snapClassIG extends nxs_snapClassNT {   
  ...
  var $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'msgTFormat'=>'', 'defImg'=>'', 'loc'=>'', 'msgFormat'=>"%TITLE% \n%HTAGS%");
  ...
  function accTab($ii, $options, $isNew=false){ $ntInfo = $this->ntInfo; $nt = $ntInfo['lcode']; 
    $this->elemUserPass($ii, $options['uName'], $options['uPass']); if (empty($options['defImg'])) $options['defImg'] = ''; if (empty($options['session'])) $options['session'] = ''; $options = array_merge($this->defO, $options); ?>       
  ...
  function setNTSettings($post, $options){ 
    foreach ($post as $ii => $pval){       
      if (!empty($pval['uPass']) && !empty($pval['uPass'])){ if (!isset($options[$ii])) $options[$ii] = array(); $options[$ii] = $this->saveCommonNTSettings($pval,$options[$ii]);    
        ...
        if (isset($pval['loc'])) $options[$ii]['loc'] = trim($pval['loc']);         
