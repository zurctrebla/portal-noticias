...
if (!class_exists("nxs_snapClassSC")) { class nxs_snapClassSC extends nxs_snapClassNT { 
  var $ntInfo = array('code'=>'SC', 'lcode'=>'sc', 'name'=>'Scoop.It', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/instructions/scoopit-social-networks-auto-poster-setup-installation/');      
  //#### Update
...
<div id="nxs_<?php echo esc_attr($nt); ?>_apinxdiv_<?php echo esc_attr($ii); ?>" class="nxs_<?php echo esc_attr($nt); ?>_apidiv_<?php echo esc_attr($ii); ?> nxs_<?php echo esc_attr($nt); ?>_apinxdiv_<?php echo esc_attr($ii); ?>" style="display: <?php echo (!empty($options['apiToUse']) && $options['apiToUse'] =='nx')?"block":"none"; ?>;"><h3>NextScripts API</h3>
    ...
    <?php if (class_exists('nxsAPI_SC')) { if (!empty($options['uPass'])&&!empty($options['uName'])) { $opNm = 'nxs_snap_sc_'.sha1('nxs_snap_sc'.$options['uName'].$options['uPass']); $opVal = nxs_getOption($opNm); //prr($opVal);
      ...
      if(!empty($options['uPass'])){ $_POST['u']=$options['uName']; $_POST['p']=$options['uPass']; $_POST['ii']=$ii; $ntw[$nt][$ii]=$options;  }$_POST = $tPST; }
    ...
    <?php } else { nxs_show_noLibWrn('"NextScripts API Library for Scoop.It" is NOT installed'); } ?>    
  ...
  function setNTSettings($post, $options){ 
    foreach ($post as $ii => $pval){       
      if (!empty($pval['appKey']) || !empty($pval['uPass'])){ if (!isset($options[$ii])) $options[$ii] = array(); $options[$ii] = $this->saveCommonNTSettings($pval,$options[$ii]);
        //## Uniqe Items        
  ...
  function adjPublishWP(&$options, &$message, $postID){ 
    if (!empty($postID)) { $postType = $options['postType']; $imgURL = '';
