    ...
    function doPostToNT($options, $message){ $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>''); if (!class_exists("nxsAPI_IG")){ $badOut['Error'] .= "Instagram API not found"; return $badOut; } 
      ...
      if (!$loginErr) $ret = $nt->post($msg, $imgURL, $options['imgAct'],$options['loc']); else { 
          if ($loginErr=='cpt'){
            ...
            } else $loginErr = 'Instagram asked you to enter the security code. Please go to the account settings and click "Submit Test Post" to get and enter code.';
          } $badOut['Error'] .= 'Something went wrong - '.print_r($loginErr, true); $ret = $badOut; 
      }            
      //## Save Login Info
