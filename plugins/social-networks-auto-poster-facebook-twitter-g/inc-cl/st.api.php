...
if (!class_exists("nxs_class_SNAP_ST")) { class nxs_class_SNAP_ST {
    ...
    function createFile($imgURL) {
        ...
        rename($tmp, $tmp.='.png'); register_shutdown_function('unlink', $tmp);
...
if (!class_exists('nxs_Aes')) { class nxs_Aes {
    public static function cipher($input, $w) {    // main cipher function [v5.1]
        $Nb = 4;                 // block size (in words): no of columns in state (fixed at 4 for AES)
        $Nr = count($w)/$Nb - 1; // no of rounds: 10/12/14 for 128/192/256-bit keys

        $state = array();  // initialise 4xNb byte-array 'state' with input [v3.4]
        ...
        $output = array(4*$Nb);  // convert state to 1-d array before returning [v3.4]
        for ($i=0; $i<4*$Nb; $i++) $output[$i] = $state[$i%4][floor($i/4)];
        return $output;
    }
    private static function addRoundKey($state, $w, $rnd, $Nb) {  // xor Round Key into state S [v5.1.4]
        for ($r=0; $r<4; $r++) {
            for ($c=0; $c<$Nb; $c++) $state[$r][$c] ^= $w[$rnd*4+$c][$r];
        }
        return $state;
    }
    private static function subBytes($s, $Nb) {    // apply SBox to state S [v5.1.1]
        for ($r=0; $r<4; $r++) {
            for ($c=0; $c<$Nb; $c++) $s[$r][$c] = self::$sBox[$s[$r][$c]];
        }
        return $s;
    }
    private static function shiftRows($s, $Nb) {    // shift row r of state S left by r bytes [v5.1.2]
        $t = array(4);
        for ($r=1; $r<4; $r++) {
    ...
    private static function mixColumns($s, $Nb) {   // combine bytes of each col of state S [v5.1.3]
        for ($c=0; $c<4; $c++) {
            $a = array(4);  // 'a' is a copy of the current column from 's'
            $b = array(4);  // 'b' is av{02} in GF(2^8)
            for ($i=0; $i<4; $i++) {
                $a[$i] = $s[$i][$c];
                $b[$i] = $s[$i][$c]&0x80 ? $s[$i][$c]<<1 ^ 0x011b : $s[$i][$c]<<1;
            }
            // a[n] ^ b[n] is av{03} in GF(2^8)
    ...
    public static function keyExpansion($key) {  // generate Key Schedule from Cipher Key [v5.2]
        $Nb = 4;              // block size (in words): no of columns in state (fixed at 4 for AES)
...
if (!class_exists('nxs_AesCtr')) { class nxs_AesCtr extends nxs_Aes {
    public static function encrypt($plaintext, $password, $nBits) {
        ...
        // initialise 1st 8 bytes of counter block with nonce (NIST SP800-38A vB.2): [0-1] = millisec,
