...
if (!class_exists("nxs_class_SNAP_TR")) { class nxs_class_SNAP_TR {
    ...
    function doPostToNT($options, $message){ $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>''); 
      ...
      //## Post    
      if (empty($options['pgID'])) { $options['trURL'] = trim(str_ireplace('https://', '', $options['trURL'])); if (substr($options['trURL'],-1)=='/') $options['pgID'] = substr($options['trURL'], 0, -1);}
      ...
      if ($options['postType']=='I') { $postArr['type'] = 'photo'; $postArr['caption'] = $msg;  $postArr['source'] = str_ireplace(' ','%20',$imgURL);
        ...
        $embedTxt = '<iframe width="560" height="315" src="https://www.youtube.com/embed/'.$message['videoURL'].'" frameborder="0" allowfullscreen></iframe>';
