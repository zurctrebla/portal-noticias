...
if (!class_exists("nxs_class_SNAP_LJ")) { class nxs_class_SNAP_LJ {
    ...
    function doPostToNT($options, $message){ $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>'');
      ...
      $nxsToLJclient = new NXS_XMLRPC_Client('https://www.'.$server.'/interface/xmlrpc'); $nxsToLJclient->debug = false;// prr($options);
      if (!empty($options['proxy'])&&!empty($options['proxyOn'])){ $nxsToLJclient->proxy['proxy'] = $options['proxy']['proxy']; if (!empty($options['proxy']['up']))  $nxsToLJclient->proxy['up'] = $options['proxy']['up']; }

        $msg = preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', "", $msg);
      
      $date = !empty($message['postDate']) ? strtotime($message['postDate']) : time(); $year = date("Y", $date); $mon = date("m", $date); $day = date("d", $date); $hour = date("G", $date); $min = date("i", $date);
