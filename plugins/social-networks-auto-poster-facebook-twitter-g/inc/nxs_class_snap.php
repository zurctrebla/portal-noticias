...
if (!class_exists("nxs_SNAP")) { class nxs_SNAP {//## SNAP General Class         
  var $dbOptionsName = "nxsSNAPOptions"; var $dbNtsName = "nxsSNAPNetworks"; var $dbNtsNameU = "nxsSNAPNetworksU"; var $nxs_snapAvNts; var $nxs_options = ""; var $nxs_ntoptions = array(); var $nxs_accts = ""; var $nxs_acctsU = ""; var $accts;
  var $sMode = array('s'=>'S', 'l'=>'F', 'u'=>'O', 'a'=>'S', 's'=>'S', 't'=>false); var $old_dbOptionsName = "NS_SNAutoPoster"; var $addns=[];
        
  function __construct($u='') { load_plugin_textdomain('social-networks-auto-poster-facebook-twitter-g', FALSE, substr(dirname( plugin_basename( __FILE__ ) ), 0, -4).'/lang/'); 
    if (empty($u)) { $u = wp_get_current_user(); $u = $u->ID; } $this->dbNtsNameU .= $u;  $this->getAPOptions(); $this->sMode['r'] = false;
    $this->sMode['s'] = (defined('MULTISITE') && MULTISITE==true)?'M':'S'; $snapMgmt = new nxs_adminMgmt(); 
    $this->sMode['l'] = function_exists('nxs_doSMAS1')?(function_exists("ns_SMASV4M1")?'M':'P'):'F';  $this->sMode['t'] = $this->sMode['l']=='M' || $this->sMode['l']=='P';

    if ($this->sMode['s']=='M'){ global $blog_id; add_action('network_admin_menu', array($snapMgmt,'ntAdminMenu')); 
      if ($this->sMode['l']!='M') { if (get_site_option('nxs_nts')==$blog_id) { $this->sMode['r'] = true; $snapMgmt->init(); } else $this->sMode['a']='I'; }
      if ($this->sMode['l']=='M') { $s = (!empty($this->nxs_ntoptions['l']))?$this->nxs_ntoptions['l']:array();  $this->sMode['u'] = !empty($s[$blog_id])?$s[$blog_id]:(!empty($this->nxs_ntoptions['nxsSUType'])?$this->nxs_ntoptions['nxsSUType']:'O'); 
        if ($this->sMode['u']=='O') { $this->sMode['r'] = true; $snapMgmt->init(); }           
        if ($this->sMode['u']=='S') { $this->sMode['r'] = true; switch_to_blog(1); $this->getAPOptions(); restore_current_blog(); }           
      }
    } else { $this->sMode['r'] = true; $snapMgmt->init(); } //  prr($this->nxs_ntoptions);
    //QP Post Type
    //$this->conns = new nxs_ntConns(); //prr($this->conns);
  }
  ...
  function toLatestVer($options){  global $nxs_snapAvNts; if (!empty($options['v'])) $v = $options['v']; else $v = 340; $optionsOut = array(); // prr($v);
     switch ($v) {
     case 340: 
       //## Networks
       $nts = array(); foreach ($nxs_snapAvNts as $avNt) {
         if (!empty($options[$avNt['lcode']])) foreach ($options[$avNt['lcode']] as $aNt) { 
           if (!empty($aNt)){ $clName = 'nxs_snapClass'.$avNt['code']; $ntt = new $clName; if (method_exists($ntt,'toLatestVer')) $nts[$avNt['lcode']][] = $ntt->toLatestVer($aNt); else $nts[$avNt['lcode']][] = $aNt; }
       ...
       if (!empty($options['lku'])) { $l = '__plugins_cache_244'; $k = get_site_option($l); if (empty($k)) { $k = array('lku'=>$options['lku'], 'ukver'=>$options['ukver'], 'uklch'=>$options['uklch']); update_site_option($l, $k); }}
       
       if (defined('MULTISITE') && MULTISITE!=false) { $ntoptions = get_site_option('NS_SNAutoPoster'); if (!empty($ntoptions)) { $ntoOut = array(); $ntoOut['nxsSUType'] = $ntoptions['nxsSUType']; $args = array('fields'=>'ids');          
         if (function_exists('get_sites')) $sites = get_sites($args); else { $sitesX = wp_get_sites(array('public'=> 1, 'archived' => 0, 'mature' => 0, 'spam' => 0, 'deleted' => 0)); $sites = array(); foreach ($sitesX as $ss) $sites[] = $ss['blog_id'];  }
         foreach ($sites as $st) { switch_to_blog( $st ); $o = get_option('NS_SNAutoPoster'); if (!empty($o['suaMode'])) $ntoOut['l'][$st] = $o['suaMode']; } restore_current_blog();
         delete_site_option('NS_SNAutoPoster'); update_site_option($this->dbOptionsName, $ntoOut);
       }} 
     break;          
  ...
  function getAPOptions() { global $nxs_skipSSLCheck, $blog_id; $options = get_option($this->dbOptionsName); $this->nxs_accts = get_option($this->dbNtsName); $this->nxs_acctsU = get_option($this->dbNtsNameU); //$this->accts = $this->rfrmAccts($this->nxs_accts); prr($this->accts); die();
	...
	$dir = $dir[0]; $pf = $dir.$nxs_UPPath.'/'.$nxs_UPPath.'.php'; if (file_exists($pf) && !class_exists('nxs_wpAPIEngine') && !class_exists('nxs_wpAddnEngine') ) require_once $pf;

    if (defined('NextScripts_UPG_SNAP_Version') && version_compare(NextScripts_UPG_SNAP_Version, '2.0.0', '<') ) {
	    if (empty( get_option( 'nxs-upg200-notice-dismissed' ))) add_action( 'admin_notices', 'nxs_admin_notice__wrongProHelper' );
	    if (class_exists('nxs_wpAPIEngine')) { $cl = new nxs_wpAPIEngine(); $cl->check(); }
    } else {  if (class_exists('nxs_wpAddnEngine')) { $cl = new nxs_wpAddnEngine(); $cl->check(); } }
    ...
    if (isset($_GET['page']) && $_GET['page']=='nxs-help' && isset($_GET['do']) && $_GET['do']=='test'){
        ...
        nxs_cURLTest("https://api.twitter.com/2/users/", "HTTPS to Twitter V2 API", 'Unauthorized');
        nxs_cURLTest("https://api.twitter.com/1.1/statuses/update.json", "HTTPS to Twitter V1.1 API", 'Authentication');
  ...
  function showUsersSitesMUTab(){ global $nxs_snapAvNts;  $ntoptions = get_site_option('nxsSNAPOptions'); ?>
   ...
   <div style="margin-left: 25px;">
      <input type="radio" onchange="nxs_saveNewSiteDefSets(this.value);"  <?php if (isset($ntoptions['nxsSUType']) && $ntoptions['nxsSUType']=='S') echo ' checked="checked" '; ?> name="nxsSUType" value="S" /> <b>Super Admin Mode.</b> <i>Super Admin configures all Social Networks. All posts made in all blogs will be re-posted to all neteworks. Blog owners and other users don't see and can't change any settings or options</i><br/>        
      <input type="radio" onchange="nxs_saveNewSiteDefSets(this.value);" <?php if (isset($ntoptions['nxsSUType']) && $ntoptions['nxsSUType']=='O') echo ' checked="checked" '; ?> name="nxsSUType" value="O" /> <b>Regular Mode</b> <i>Blog owners can configure and autopost to their own Social Networks</i><br/>   
      <input type="radio" onchange="nxs_saveNewSiteDefSets(this.value);" <?php if ( (!isset($ntoptions['nxsSUType']) || isset($ntoptions['nxsSUType']) && $ntoptions['nxsSUType']=='D')) echo ' checked="checked" '; ?> name="nxsSUType" value="D" /> <b>Disabled</b> <i>SNAP is disabled for new sites.</i><br/>
      </div>      
...
&nbsp;&nbsp;<a style="font-size: 14px;" onclick="nxs_saveSiteSets('A', 'D');return false;" href="#">[Disable SNAP on all sites]</a><?php
  ...
  function showAccountsTab(){ global $nxs_snapAvNts, $nxsOne; $nxsOne = ''; $trrd=0; $parts = parse_url( home_url() );
      ...
      if ($secCheck!==false && isset($_FILES['impFileSettings_button']) && is_uploaded_file($_FILES['impFileSettings_button']['tmp_name'])) { $fileData = trim(file_get_contents($_FILES['impFileSettings_button']['tmp_name']));
        ...
        $uplOpt = maybe_unserialize($fileData); if (is_array($uplOpt) && (isset($uplOpt['imgNoCheck']) || isset($uplOpt['useSSLCert'])) ) { $options = $uplOpt; //### V3 import
          if (!empty($options)&&(empty($options['v'])||($options['v']<NXS_SETV))) { if (empty($options['v'])) add_action( 'admin_enqueue_scripts', 'nxs_snap_pointer_admin_enqueue_scripts' ); $options = $this->toLatestVer($options);  }  //## Check if first run after V3-V4 update.
            ...
            if (!empty($_POST['nxs_doAccMrg'])) { $networks = (!current_user_can( 'manage_options' ) && current_user_can( 'haveown_snap_accss' ) ) ? $this->nxs_acctsU : $this->nxs_accts; 
    ...
    $networks = (!current_user_can( 'manage_options' ) && current_user_can( 'haveown_snap_accss' ) ) ? $this->nxs_acctsU : $this->nxs_accts;  $options = $this->nxs_options; $isNoNts = true; 
    //## Get rid of empty networks
    $nt2=array();if(!empty($networks)&&is_array($networks))foreach($networks as $ntnt=>$ntas)if(!empty($ntas)&&is_array($ntas))foreach($ntas as $ntii=>$nta)if(isset($ntii)&&$ntii!=='')$nt2[$ntnt][$ntii]=$nta;if($nt2!=$networks){$networks=$nt2;$this->saveNetworksOptions($nt2);}
    ...
    ?> <form method="post" id="nsStForm" action=""><input type="hidden" name="nxsMainFromSupportFld" id="nxsMainFromSupportFld" value="1" />
      ...
      <?php if (!$isMobile) {?><div class="nxsInfoMsg"><img style="position: relative; top: 8px;" alt="Arrow" src="<?php echo NXS_PLURL; ?>img/arrow_l_green_c1.png"/> You can add Facebook, Twitter, Telegram, Pinterest, LinkedIn, Tumblr, Blogger, ... accounts</div><?php } ?><br/>
      ...
      <?php  if (empty($options['showNTListCats'])) foreach ($nxs_snapAvNts as $avNt) { if (!isset($networks[$avNt['lcode']]) || count($networks[$avNt['lcode']])==0) $mt=0; else $mt = 1+max(array_keys($networks[$avNt['lcode']]));
       ...
       <?php } else { ?>
         ...
         <div style="margin-top: 10px;">
           <span><img style="padding: 3px;top: 5px;position: relative;" src="<?php echo NXS_PLURL; ?>img/cbch.png"/><?php _e('Checked checkbox - posts will be autoposted to that network by default', 'social-networks-auto-poster-facebook-twitter-g') ?><br/></span>
           <span><img style="padding: 3px;top: 5px;position: relative;" src="<?php echo NXS_PLURL; ?>img/cbun.png"/><?php _e('Unchecked checkbox - posts will NOT be autoposted to that network by default', 'social-networks-auto-poster-facebook-twitter-g') ?><br/></span>
           <span><img style="padding: 3px;top: 5px;position: relative;" src="<?php echo NXS_PLURL; ?>img/cbrb.png"/><?php _e('Radiobutton - Filters are on. Posts will be autoposted or not autoposted depending on filters', 'social-networks-auto-poster-facebook-twitter-g') ?><br/></span>           
         </div>
...
<button onclick="document.getElementById('nxs_myDropdown').classList.toggle('nxs_show');" class="nxs_dropbtn">Import/Export Accounts</button>
  ...
  function showSettingsTab($useSec=true) { global $nxs_snapAvNts, $snap_curPageURL, $nxs_snapThisPageUrl, $nxsOne, $nxs_isWPMU, $nxs_tpWMPU; $nxsOne = ''; $options = $this->nxs_options; ?>
...
<?php    foreach ( $editable_roles as $role => $details ) { $name = translate_user_role($details['name'] ); echo '<input type="checkbox" '; 
    ...
    foreach ( $editable_roles as $role => $details ) { $name = translate_user_role($details['name'] ); echo '<input type="checkbox" '; 
        ...
        echo '<br/>';    
            ...
            <div class="nxs_box_inside"> <span style="font-size: 11px; margin-left: 1px;"><?php _e('How to show the Networks List in the "Add New Network" dropdown', 'social-networks-auto-poster-facebook-twitter-g') ?></span> <br/>
              ...
              &nbsp;&nbsp;&nbsp;<input type="radio" name="howToShowNTS" value="C" <?php if (!empty($options['howToShowNTS']) && $options['howToShowNTS']=='C') echo 'checked="checked"'; ?> /> <b><?php _e('Collapsed', 'social-networks-auto-poster-facebook-twitter-g') ?>Collapsed.</b><br/>
             ...
             &nbsp;&nbsp;&nbsp;<input value="set" id="hideUnchecked" name="hideUnchecked"  type="checkbox" <?php if (!empty($options['hideUnchecked']) && (int)$options['hideUnchecked'] == 1) echo "checked"; ?> />  <b><?php _e('Hide unchecked networks', 'social-networks-auto-poster-facebook-twitter-g') ?></b>     
             &nbsp;&nbsp;&nbsp;<input value="set" id="hideFiltered" name="hideFiltered"  type="checkbox" <?php if (!empty($options['hideFiltered']) && (int)$options['hideFiltered'] == 1) echo "checked"; ?> />  <b><?php _e('Hide Filtered Out networks', 'social-networks-auto-poster-facebook-twitter-g') ?></b>     
   ...
   if (isset($options['useForPages']) && $options['useForPages'] =='1') { $options['fltrs']['nxs_post_type'][] = 'page'; unset($options['useForPages']); } ?>
...
<input name="riHowManyPostsToTrack" style="width: 50px;" value="<?php if (isset($options['riHowManyPostsToTrack'])) _e(apply_filters('format_to_edit', $options['riHowManyPostsToTrack']), 'social-networks-auto-poster-facebook-twitter-g'); else echo "10"; ?>" /> <br/>
             ...
             <span style="font-size: 11px; margin-left: 1px;">Setting too many will degrade your website's performance. 10-20 posts are recommended</span> 
              </div>
              <div class="itemDiv">  
 ...
 <br/>
     ...
     <!-- ##################### Open Graph #####################-->
            <div class="nxs_box"> <div class="nxs_box_header"><h3><?php _e('"Open Graph" Tags', 'social-networks-auto-poster-facebook-twitter-g') ?></h3></div>
             <div class="nxs_box_inside"> <span style="font-size: 11px; margin-left: 1px;"><?php _e('SNAP has a very simple implementation of the "Open Graph" metatags. Please see here for more info - SNAP Open Graph Tags', 'social-networks-auto-poster-facebook-twitter-g') ?> </span> <br/>              
              <div class="itemDiv"> <?php  
                $ogMsgT =  __('plugin has been detected', 'social-networks-auto-poster-facebook-twitter-g').' '.__('It has support for OG Metatags. Please disable SNAP OG Tags to avoid conflicts.', 'social-networks-auto-poster-facebook-twitter-g'); $ogMsg = '';
                
                if (function_exists('wpfbogp_start_ob')) $ogMsg = '"WP Facebook Open Graph protocol"';
                if (function_exists('webdados_fb_run')) $ogMsg = '"Facebook Open Graph, Google+ and Twitter Card Tags"';
                if (function_exists('fbogmeta_header')) $ogMsg = '"Facebook Featured Image and Open Graph Meta Tags"';
                if (function_exists('sfogi_wp_head')) $ogMsg = '"Simple Facebook OG image"';
                
                if (class_exists('Ngfb')) $ogMsg = '"NextGEN Facebook"';
                if (class_exists('Wpsso')) $ogMsg = '"WPSSO (Core Plugin)"';
                if (class_exists('Open_Graph_Pro')) $ogMsg = '"Open Graph Pro"';
                if (class_exists('NY_OG_Output')) $ogMsg = '"WP Open Graph"';
                if (class_exists('iworks_opengraph')) $ogMsg = '"OG (Very tiny Open Graph plugin)"';                
                if (class_exists('OpenGraphMetabox')) $ogMsg = '"Open Graph Metabox"';
                
                if (function_exists('wpseo_auto_load')) $ogMsg = '"Yoast SEO"';                
                if (function_exists('aioseop_activate')) $ogMsg = '"All In One SEO Pack"';                
                if (function_exists('the_seo_framework_pre_load')) $ogMsg = '"The SEO Framework"';
                
                
                if (!empty($ogMsg)) echo '<div style="color:darkred;">'.$ogMsg.' '.$ogMsgT.'</div></br>';
              ?>
              <input value="A" id="nsOpenGraph" name="nxsOG"  type="radio" <?php if ($options['nxsOG'] == 'A') echo "checked"; ?> /> <b><?php _e('Add Open Graph Tags', 'social-networks-auto-poster-facebook-twitter-g') ?></b><br/>
              <input value="N" id="nsOpenGraph" name="nxsOG"  type="radio" <?php if ($options['nxsOG'] == 'N') echo "checked"; ?> /> <b><?php _e('Add Open Graph Tags only when nessesary', 'social-networks-auto-poster-facebook-twitter-g') ?></b><br/>
              <input value="D" id="nsOpenGraph" name="nxsOG"  type="radio" <?php if ($options['nxsOG'] == 'D') echo "checked"; ?> /> <b><?php _e('Do not add Open Graph Tags', 'social-networks-auto-poster-facebook-twitter-g') ?></b><br/>
              </div>                           
              <div class="itemDiv">
            ...
            <input name="ogImgDef" style="width: 30%;" value="<?php if (isset($options['ogImgDef'])) _e(apply_filters('format_to_edit',$options['ogImgDef']), 'social-networks-auto-poster-facebook-twitter-g') ?>" />
             ...
             <input name="ogAuthorFB" style="width: 30%;" value="<?php if (isset($options['ogAuthorFB'])) _e(apply_filters('format_to_edit',$options['ogAuthorFB']), 'social-networks-auto-poster-facebook-twitter-g') ?>" />
              </div>              
           </div></div>    
            <!-- #### "Featured" Image ##### --> 
      ...
      <!-- ##### Alternative "Featured Image" location ##### --> 
            ...
            <div class="nxs_box"> <div class="nxs_box_header"><h3><?php _e('System Settings', 'social-networks-auto-poster-facebook-twitter-g') ?></h3></div>
             ...
             <div class="itemDiv">
                ...
                <input name="queryInterval" style="width: 50px;" value="<?php if (isset($options['queryInterval'])) _e(apply_filters('format_to_edit',$options['queryInterval']), 'social-networks-auto-poster-facebook-twitter-g'); else echo "60"; ?>" />
             ...
             <div class="itemDiv">
                ...
                <input name="numOfTasks" style="width: 50px;" value="<?php if (isset($options['numOfTasks'])) _e(apply_filters('format_to_edit',$options['numOfTasks']), 'social-networks-auto-poster-facebook-twitter-g'); else echo "30"; ?>" />
             ...
             <div class="itemDiv">
               ...
               <input name="numLogRows" style="width: 50px;" value="<?php if (isset($options['numLogRows'])) _e(apply_filters('format_to_edit',$options['numLogRows']), 'social-networks-auto-poster-facebook-twitter-g'); else echo "1000"; ?>" />
               <br/>
              </div>
  ...
  function showLogHistoryTab() { global $nxs_snapAvNts, $nxsOne, $nxs_isWPMU, $nxs_tpWMPU; $nxsOne = ''; $options = $this->nxs_options; 
  ...
  function showQueryTab() { global $wpdb, $nxs_snapAvNts, $nxsOne, $nxs_isWPMU, $nxs_tpWMPU; $nxsOne = ''; $options = $this->nxs_options; 
    ...
    <a href="#" style="float: right" onclick="nxs_rfLog();return false;" class="NXSButton" id="nxs_clearLog">Refresh</a>
      <?php if (!is_array($quPosts)) $quPosts = array(); if (count($quPosts)>0) { ?><div>
      <?php _e('Future Posts Timeline', 'social-networks-auto-poster-facebook-twitter-g') ?>&nbsp;(<?php _e('Time Now:', 'social-networks-auto-poster-facebook-twitter-g');  echo '&nbsp;'.date_i18n('Y-m-d H:i'); ?>)
      ...
      <?php } else _e('Query is empty', 'social-networks-auto-poster-facebook-twitter-g'); //prr($quPosts); 
  ...
  function showAboutTab(){ global $nxs_snapAvNts, $nxs_snapThisPageUrl, $nxsOne, $nxs_isWPMU, $nxs_tpWMPU; $nxsOne = ''; $options = $this->nxs_options; 
    ...
    <div style="max-width:1000px;">
        <?php   nxs_addns::showAddnsListBlock(); nxs_addns::showAddnsActButtons(); ?>
        <div class="nxscontainer">
          <div class="nxsleft">
            <h3 style="margin-top: 0px; padding-left: 0px; font-size: 16px;"><?php _e('System Tests', 'social-networks-auto-poster-facebook-twitter-g');?></h3>
            <h5 style="margin-top: 5px; margin-bottom: 2px; padding-left: 0px; font-size: 16px;"><?php _e('System', 'social-networks-auto-poster-facebook-twitter-g');?></h5>
            <div style="padding-bottom: 10px;"><?php nxs_memCheck(); ?></div>
            <h5 style="margin-top: 5px; margin-bottom: 2px; padding-left: 0px; font-size: 16px;"><?php _e('Cron', 'social-networks-auto-poster-facebook-twitter-g');?></h5>
            
            <div style="padding-bottom: 10px;">Internal WP Cron execution:&nbsp;<?php if (defined('DISABLE_WP_CRON') && DISABLE_WP_CRON==true) echo "Disabled - OK"; else echo "Enabled - OK, but not reccomended"; ?><br/>
            SNAP Query Cron Event:&nbsp;<?php if (wp_next_scheduled('nxs_querypost_event')) echo "Shedulled - OK"; else echo "NOT Found"; ?>&nbsp;(Last Cron Execution:&nbsp;<?php $tmCorr = get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ;  $tmL = get_option('nxs_last_nxs_cron'); echo date_i18n('Y-m-d H:i:s',$tmL+$tmCorr)." - "; $tm = microtime(); $tma = explode(' ',$tm); $tm = number_format((float)$tma[0]+(float)$tma[1], 8, '.', ''); $currTime = $tm+$tmCorr; echo number_format(($tm-$tmL), 2,'.','').'sec ago';
            ...
            ?><a style="font-weight: normal; font-size: 16px; line-height: 24px;" target="_blank" href="<?php echo $nxs_snapThisPageUrl; ?>&do=crtest">Show Cron Test Results</a><br/></div>
              ...
              <a style="font-weight: normal; font-size: 14px; line-height: 24px;" target="_blank" href="https://www.nextscripts.com/instructions/"><?php _e('Setup/Installation Instructions for each network', 'social-networks-auto-poster-facebook-twitter-g');?></a><br/>
              <a style="font-weight: normal; font-size: 14px; line-height: 24px;" target="_blank" href="https://www.nextscripts.com/tutorials/"><?php _e('Setup/Configuration/Usage Tutorials', 'social-networks-auto-poster-facebook-twitter-g');?></a><br/>

              <h3 style="margin-top: 20px; padding-left: 0px; font-size: 14px;"><?php _e('General Questions', 'social-networks-auto-poster-facebook-twitter-g');?></h3>
              <a style="font-weight: normal; font-size: 14px; line-height: 24px;" target="_blank" href="https://www.nextscripts.com/faq">FAQ</a><br/>

              <h3 style="margin-top: 20px; padding-left: 0px; font-size: 14px;"><?php _e('Solutions for the most common problems', 'social-networks-auto-poster-facebook-twitter-g');?></h3>
              <a style="font-weight: normal; font-size: 14px; line-height: 24px;" target="_blank" href="https://www.nextscripts.com/support-faq/">Troubleshooting FAQ</a><br/>

              <h3 style="margin-top: 20px; padding-left: 0px; font-size: 14px;"><?php _e('Have troubles/problems/found a bug?', 'social-networks-auto-poster-facebook-twitter-g');?></h3>
              <a style="font-weight: normal; font-size: 14px; line-height: 24px;" target="_blank" href="https://www.nextscripts.com/support">===&gt; Open support ticket &lt;===</a>


              <h3 style="margin-top: 20px; padding-left: 0px; font-size: 14px;"><?php _e('Have questions/suggestions?', 'social-networks-auto-poster-facebook-twitter-g');?></h3>
              <a style="font-weight: normal; font-size: 14px; line-height: 24px;" target="_blank" href="https://www.nextscripts.com/contact-us">===&gt; Contact us &lt;===</a> <br/>

              <h3 style="margin-top: 20px; padding-left: 0px; font-size: 14px;;"><?php _e('Like the Plugin? Would you like to support developers?', 'social-networks-auto-poster-facebook-twitter-g');?></h3>
              <div style="line-height: 24px;">
                  <b>Here is what you can do:</b><br/>
		          <?php if(class_exists('nxaddn_ma')) { ?><s><?php } ?><img src="<?php echo NXS_PLURL; ?>img/snap-icon12.png"/> Get few <a target="_blank" href="https://www.nextscripts.net/">Addons</a>. You will be able to add several accounts for each network as well as post to additional networks like Google My Business, Pinterest and LinkedIn company pages.<?php if(class_exists('nxaddn_ma')) { ?></s> <i>Done! Thank you!</i><?php } ?><br/>
...
&nbsp;&nbsp;<a id="nxs_deleteAllSNAPInfo" <?php echo is_network_admin()?'data-mu="mu"':''; ?> href="#">[<?php _e('Delete all SNAP Data', 'social-networks-auto-poster-facebook-twitter-g');?>]</a> - <?php _e('This is a complete "Start Over". This will delete all SNAP data from the posts and all SNAP settings including all configured networks.', 'social-networks-auto-poster-facebook-twitter-g');?>
	...
	function showAddonsTab(){
		$this->addns = nxs_addns::getAddnList(); update_site_option('_nxs_aListV4', $this->addns); nxs_addns::$lst = json_decode($this->addns, true); nxs_addns::showAddnsPage('4');
	}
        
  function NS_SNAP_AddPostMetaTags() { global $post, $nxs_snapAvNts; $post_id = $post; if (is_object($post_id))  $post_id = $post_id->ID; 
          if (!is_object($post) || empty($post->post_status)) $post = get_post($post_id); $postType = $post->post_type;
       ...
       <?php /* ################## WHAT URL to USE */ ###################### ?>
          ...
          jQuery(document).ready(function($) {<?php  if (!empty($options['howToShowNTS']) && $options['howToShowNTS']=='C') { ?> 
            jQuery('.nxsEdPostMetaBlockBody').hide();  jQuery('.nxs_ldos').html('[+]');  <?php } elseif (!empty($options['howToShowNTS']) && $options['howToShowNTS']=='E') { ?> jQuery('.nxsEdPostMetaBlockBody').show();  jQuery('.nxs_ldos').html('[-]'); 
          <?php } ?>
          });</script>
          ...
          foreach ($nxs_snapAvNts as $avNt) { $clName = 'nxs_snapClass'.$avNt['code'];
             ...
             $cbo = 0; $cboInx = 0; $jXj = 0; foreach ($ntClInst->nt as $indx=>$pbo){ 
               if (!empty($pbo['fltrsOn']) && $pbo['fltrsOn'] == '1') { if (empty($pbo['fltrs'])) $pbo['fltrs'] = array(); if (empty($pbo['fltrs']['nxs_post_type'])) $pbo['fltrs']['nxs_post_type'] = array('post'); 
                 $fltPostTypeExcl = !in_array($postType,$pbo['fltrs']['nxs_post_type']); if (!empty($pbo['fltrs']['nxs_ie_posttypes'])) $fltPostTypeExcl = !$fltPostTypeExcl;
               } else $fltPostTypeExcl = false;//  prr($indx, 'IDX'); prr($pbo, 'PBO');
               ...
               $pbo['hideMe'] = (!empty($options['hideUnchecked']) && empty($pbo['do'])) || ( !empty($options['hideFiltered']) && $fltPostTypeExcl );  if (!$pbo['hideMe']) $jXj++;  
             } if ($cbo==0) continue; if (!$this->sMode['t']) $cbo = 1;
             if ($cbo==1) { ?>
               <?php  $pbo = $ntClInst->nt[$cboInx]; $pbo['jj']=$cboInx; $pbo['cbo']=$cbo;  $pbo['hideMe'] = (!empty($options['hideUnchecked']) && empty($pbo['do'])) || ( !empty($options['hideFiltered']) && $fltPostTypeExcl ); 
             ...
             <div class="nxs_box" <?php if ($jXj==0) echo ' style="display:none;" '; ?> onmouseover="jQuery('.selAll<?php echo $avNt['code']; ?>').show();" onmouseout="jQuery('.selAll<?php echo $avNt['code']; ?>').hide();">
               ...
               <div class="nxs_box_inside"><?php $jj = 0;  if(!$ntClInst->checkIfFunc()) echo $ntClInst->noFuncMsg; //### List of accountns
                     ...
                     if (!empty($pbo['fltrsOn']) && $pbo['fltrsOn'] == '1') { if (empty($pbo['fltrs'])) $pbo['fltrs'] = array(); if (empty($pbo['fltrs']['nxs_post_type'])) $pbo['fltrs']['nxs_post_type'] = array('post'); 
                        $fltPostTypeExcl = !in_array($postType,$pbo['fltrs']['nxs_post_type']); if (!empty($pbo['fltrs']['nxs_ie_posttypes'])) $fltPostTypeExcl = !$fltPostTypeExcl;
                     } else $fltPostTypeExcl = false;
                                      
                     $pbo['hideMe'] = (!empty($options['hideUnchecked']) && empty($pbo['do'])) || ( !empty($options['hideFiltered']) && $fltPostTypeExcl ); if (!$pbo['hideMe']) $jj++;
...
<input type="hidden"  id="nxs_timeID" value="" /> <input type="hidden"  id="nxs_timeID_ED" value="0" />
  ...
  function NS_SNAP_SavePostMetaTags($id) { global $nxs_snapAvNts, $nxs_SNAP;         
          ...
          $tp = get_option('_nxs_tp'); if (is_array($tp) && in_array($id, $tp)){
              nxsLogIt(array('type'=>'I', 'act'=>'Guttenberg', 'msg'=>'Got Metadata from pending post. Requesting Autopost...'.var_export($tp,true))); $tp=array_diff($tp,array($id)); update_option('_nxs_tp',$tp,false); nxs_snapPublishTo($id);
          }
        }
  //## Add MetaBox to Post->Edit
  function addCustomBoxes() { global $nxs_SNAP; if (!isset($nxs_SNAP)) return; if ($nxs_SNAP->sMode['a']=='I') return; $options = $nxs_SNAP->nxs_options;  //## Add to Post, Page and Custom Post Types
     $args=array('public'=>true, '_builtin'=>false); $output = 'names'; $operator = 'and'; $post_types = array(); 
     if (function_exists('get_post_types')) $post_types=get_post_types($args, $output, $operator); $post_types['post'] = 'post'; $post_types['page'] = 'page';
     if (empty($options['fltrs']) || !is_array($options['fltrs'])) $options['fltrs'] = array(); if (empty($options['fltrs']['nxs_post_type']) || !is_array($options['fltrs']['nxs_post_type'])) $options['fltrs']['nxs_post_type'] = array('post');        
     //prr($options['fltrs']['nxs_post_type']); prr($post_types);          
     foreach ($post_types as $cptID=>$cptName) if ((empty($options['fltrs']['nxs_ie_posttypes']) && in_array($cptID, $options['fltrs']['nxs_post_type'])) || (!empty($options['fltrs']['nxs_ie_posttypes']) && !in_array($cptID, $options['fltrs']['nxs_post_type']))){ 
  ...
  function setSettingsFromPOST(){ $options = $this->nxs_options; $pvData = nxs_Filters::sanitize_data($_POST);
            if (isset($pvData['apCats']))      $options['apCats'] = $pvData['apCats'];                
            if (isset($pvData['nxsHTDP']))     $options['nxsHTDP'] = $pvData['nxsHTDP'];                
            if (isset($pvData['ogImgDef']))    $options['ogImgDef'] = $pvData['ogImgDef'];  
            if (isset($pvData['ogAuthorFB']))    $options['ogAuthorFB'] = $pvData['ogAuthorFB'];  
            if (isset($pvData['featImgLoc']))  $options['featImgLoc'] = $pvData['featImgLoc'];            
            if (isset($pvData['imgSizeImg']))  $options['imgSizeImg'] = $pvData['imgSizeImg'];            
            if (isset($pvData['imgSizeAttch']))  $options['imgSizeAttch'] = $pvData['imgSizeAttch'];            
            if (isset($pvData['anounTagLimit']))  $options['anounTagLimit'] = $pvData['anounTagLimit'];                        
            if (isset($pvData['nxsHTSpace']))  $options['nxsHTSpace'] = $pvData['nxsHTSpace']; else $options['nxsHTSpace'] = "";
            if (isset($pvData['nxsHTSepar']))  $options['nxsHTSepar'] = $pvData['nxsHTSepar']; else $options['nxsHTSepar'] = "c_";
            if (isset($pvData['featImgLocPrefix']))  $options['featImgLocPrefix'] = $pvData['featImgLocPrefix'];
            if (isset($pvData['featImgLocArrPath']))  $options['featImgLocArrPath'] = $pvData['featImgLocArrPath'];
            
            if (isset($pvData['errNotifEmailCB']))   $options['errNotifEmailCB'] = 1;  else $options['errNotifEmailCB'] = 0;
            if (isset($pvData['errNotifEmail']))$options['errNotifEmail'] = $pvData['errNotifEmail']; 
      ...
      if (isset($pvData['YOURLSKey'])) $options['YOURLSKey'] = $pvData['YOURLSKey'];
            ...
            if (isset($pvData['forceSURL']))   $options['forceSURL'] = 1;  else $options['forceSURL'] = 0;                       
            if (isset($pvData['brokenCntFilters']))   $options['brokenCntFilters'] = 1;  else $options['brokenCntFilters'] = 0;      
            if (isset($pvData['numLogRows']))   $options['numLogRows'] = $pvData['numLogRows']; else $options['numLogRows'] = 1000;      
            if (isset($pvData['queryInterval']))   $options['queryInterval'] = $pvData['queryInterval']; else $options['queryInterval'] = 60;            
            if (isset($pvData['numOfTasks']))   $options['numOfTasks'] = $pvData['numOfTasks']; else $options['numOfTasks'] = 30;                        
            
            $numQU = intval($options['queryInterval']);  if (!is_numeric($numQU) || ($numQU < 30 || $numQU > 600)) $options['queryInterval'] = 60;
            
            if (isset($pvData['nsOpenGraph']))   $options['nsOpenGraph'] = $pvData['nsOpenGraph']; else $options['nsOpenGraph'] = 0;                
            if (isset($pvData['nxsOG']))   $options['nxsOG'] = $pvData['nxsOG']; else $options['nxsOG'] = 'N';
            
            if (isset($pvData['imgNoCheck']))   $options['imgNoCheck'] = 0;  else $options['imgNoCheck'] = 1;
            if (isset($pvData['useForPages']))  $options['useForPages'] = 1;  else $options['useForPages'] = 0;
                        
            if (isset($pvData['showPrxTab']))   $options['showPrxTab'] = 1;  else $options['showPrxTab'] = 0;
            if (isset($pvData['useRndProxy']))   $options['useRndProxy'] = 1;  else $options['useRndProxy'] = 0;
            
            if (!empty($pvData['showNTListCats']))  $options['showNTListCats'] = 1;  else $options['showNTListCats'] = 0;            
            if (!empty($pvData['howToShowNTS']))  $options['howToShowNTS'] = $pvData['howToShowNTS'];           
            ...
            if (isset($pvData['riHowManyPostsToTrack'])) $options['riHowManyPostsToTrack'] = $pvData['riHowManyPostsToTrack'];             
            if (isset($pvData['riHowOften'])) $options['riHowOften'] = $pvData['riHowOften'];             
            
            if (isset($pvData['useUnProc']))   $options['useUnProc'] = $pvData['useUnProc']; else $options['useUnProc'] = 0;    
            if (!empty($pvData['nxsCPTSeld']) && is_array($pvData['nxsCPTSeld'])) $cpTypes = $pvData['nxsCPTSeld']; else $cpTypes = array();  $options['nxsCPTSeld'] = serialize($cpTypes); 
            
            if (!isset($pvData['whoCanSeeSNAPBox'])) $pvData['whoCanSeeSNAPBox'] = array(); $pvData['whoCanSeeSNAPBox'][] = 'administrator';            
            if (isset($pvData['whoCanSeeSNAPBox'])) $options['whoCanSeeSNAPBox'] = $pvData['whoCanSeeSNAPBox']; 
            if (!isset($pvData['whoCanMakePosts'])) $pvData['whoCanMakePosts'] = array(); $pvData['whoCanMakePosts'][] = 'administrator';            
            if (isset($pvData['whoCanMakePosts'])) $options['whoCanMakePosts'] = $pvData['whoCanMakePosts']; 
            
            if (!isset($pvData['whoCanHaveOwnSNAPAccs'])) $pvData['whoCanHaveOwnSNAPAccs'] = array(); $pvData['whoCanHaveOwnSNAPAccs'][] = 'administrator';            
            if (isset($pvData['whoCanHaveOwnSNAPAccs'])) $options['whoCanHaveOwnSNAPAccs'] = $pvData['whoCanHaveOwnSNAPAccs'];
            ...
            if (isset($pvData['hideFiltered'])) $options['hideFiltered'] = 1;  else $options['hideFiltered'] = 0;
            ...
            //## Query has been activated
              ...
              if (isset($pvData['nxsOverLimit'])) $options['nxsOverLimit'] = $pvData['nxsOverLimit']; 
              if (isset($pvData['quLimitRndMins'])) $options['quLimitRndMins'] = $pvData['quLimitRndMins'];           
              if (isset($pvData['quDays'])) $options['quDays'] = $pvData['quDays']; else $options['quDays'] = 0;
              if (isset($pvData['quHrs'])) $options['quHrs'] = $pvData['quHrs']; else $options['quHrs'] = 0;
              if (isset($pvData['quMins'])) $options['quMins'] = $pvData['quMins'];else $options['quMins'] = 0;
            ...
            if (isset($pvData['rpstActive'])) $options['rpstActive'] = 1;  else $options['rpstActive'] = 0;      //     prr($options);
...
class nxs_ReposterListTable extends WP_List_Table {
    ...
    var $fltsInfo=''; var $isActive=false;
    ...
    function column_title($item){
        ...
        $actions = array(
            ...
            'delete'    => sprintf('<a href="?page=%s&action=%s&item=%s&_wpnonce=%s">Delete</a>',$pid,'delete',$item->ID, wp_create_nonce( 'nxsRepDel_'.$item->ID )),
        );
    ...
    function column_posts($item){return count($this->fltsInfo['p']); }    
    function column_filters($item){ return $this->fltsInfo['i']; /* ."<pre>".print_r($this->fltsInfo, true)."</pre>"; */ }
    
    function column_nextdate($item){ $gr2Options = maybe_unserialize(get_post_meta( $item->ID, 'nxs_rpstr', true )); //prr($gr2Options);
       if ($this->isActive && !empty($gr2Options['rpstNxTime'])) { $pNextTime =  $gr2Options['rpstNxTime']>0?date_i18n('Y-m-d H:i', $gr2Options['rpstNxTime']):'Never';       
         return $pNextTime;
       }        
       return 'n/a';
    }
...
class nxs_QPListTable extends WP_List_Table {
    ...
    function column_post_title($item){
        ...
        $actions = array(
            'edit'      => sprintf('<a href="?page=%s&action=%s&item=%s">Edit</a>',$pid,'edit',$item->ID),
            'delete'    => sprintf('<a href="?page=%s&action=%s&item=%s&_wpnonce=%s">Delete</a>',$pid,'delete',$item->ID, wp_create_nonce( 'nxsNPDel_'.$item->ID )),
        );
        //Return the title contents
    ...
    function prepare_items() { 
        ...
        $args=array('post_type' => 'nxs_qp', 'posts_per_page' => 500, 'orderby'=> 'date',  'order' => 'DESC'); if ((!current_user_can( 'manage_options' ) && current_user_can( 'haveown_snap_accss' ) )) $args['author__in'] = get_current_user_id();
 ...
 */
function nxs_qp_ajax_script() { $screen = get_current_screen(); if (stripos($screen->id,'_page_nxssnap-post')===false) return false; ?>
<script type="text/javascript">
