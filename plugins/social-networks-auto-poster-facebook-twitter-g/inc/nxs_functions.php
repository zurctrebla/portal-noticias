...
if (!function_exists('nxs_curlUploadImg')){ function nxs_curlUploadImg($imgURL, $uplURL, $pstArray, $pstField, $ck='') { $remImgURL = urldecode($imgURL); $urlParced = pathinfo($remImgURL); $remImgURLFilename = $urlParced['basename']; 
  ...
  if (function_exists('curl_file_create')) { $file  = curl_file_create($tmpX); $pstArray[$pstField] = $file; } else $pstArray[$pstField] = '@' . $tmpX;
  $argArr['extraHeaders']['Content-Type'] = 'multipart/form-data';
  $argArr['flds'] = $pstArray; $rq = new nxsHttp; $args = nxs_mkRmReqArgs($argArr); $rq->postData = $pstArray;// prr($args);
  $rep = $rq->request($uplURL, $args); @unlink($tmpX); if(is_nxs_error($rep)) return array('err'=>print_r($rep, true)); else return $rep;
}}
...
if (!function_exists("nxs_admin_notice__wrongProHelper")) { function nxs_admin_notice__wrongProHelper() { $class = 'notice notice-error is-dismissible nxs-upg200-notice';
	$message = __( 'You have "SNAP Pro Upgrade Helper" version', 'social-networks-auto-poster-facebook-twitter-g' ).' '.NextScripts_UPG_SNAP_Version.'. Please <a href="'.nxs_get_admin_url('update-core.php?force-check=1').'">upgrade</a> it to version 2.0.0 or higher to get the latest expirience.';
	$js = "<script> jQuery(document).on( 'click', '.nxs-upg200-notice .notice-dismiss', function() { jQuery.post(ajaxurl,{action: 'nxs_snap_aj','nxsact':'hide-nxs-upg200-notice', _wpnonce: jQuery('input#nxsSsPageWPN_wpnonce').val()}, 'html'); });</script>";

  printf( '<div class="%1$s"><p>%2$s</p>%3$s</div>', esc_attr( $class ),  $message, $js  );
}}              
