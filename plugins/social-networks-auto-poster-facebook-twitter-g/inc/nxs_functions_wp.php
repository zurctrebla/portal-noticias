...
if (!function_exists("nxs_doSystemInitCheck")) { function nxs_doSystemInitCheck() {  global $nxs_mLimit;
  //## Check if Pinpressr is installed
  if (function_exists('pbpPostToPinterest') && function_exists('pbp_plugin_admin_init')) { add_action( 'admin_notices', 'nxs_noPinpressrMsg' ); }
  //## Memory Check
  $nxs_mLimit = ini_get('memory_limit'); if (strpos($nxs_mLimit, 'G')) {$nxs_mLimit = (int)$nxs_mLimit * 1024;} else {$nxs_mLimit = (int)$nxs_mLimit;} 
  if ($nxs_mLimit>0 && $nxs_mLimit<64) {  add_filter( 'plugin_row_meta', 'nxs_row_meta_nomem',10,2); add_filter('plugin_action_links','ns_add_nomem_link', 10, 2 ); return false; }  
...
if (!function_exists("nxs_noPinpressrMsg")) { function nxs_noPinpressrMsg() { echo '<div class="error"><p><b>Message from NextScripts SNAP Plugin for Wordpress</b></p><p>Pinpressr plugin is using Pinterest API library stolen from NextScripts. That library is outdated and will mess up SNAP funtionlity. Please uninstall and remove Pinpressr from your system.</p></div>'; }}
if (!function_exists("ns_add_nomem_link")) { function ns_add_nomem_link($links, $file) { global $nxs_mLimit; static $this_plg; if (!$this_plg) $this_plg = plugin_basename(__FILE__);
  if ($file == $this_plg){ $sl = '<b style="color:red;">Not Active! Not Enough Memory allowed for PHP.</b> <br/> You have '.$nxs_mLimit.' MB. You need at least 64MB. (Please see <a target="_blank" href="https://www.nextscripts.com/support-faq/#a16">FAQ #1.6</a>'; array_unshift($links, $sl);} return $links;
}}
...
function nxs_row_meta($data, $page){ if ( $page != NXSSNAP_BASENAME ) { return $data; }
    return array_merge($data,array(        
        '<a href="'.  admin_url('admin.php?page=nxssnap').'" target="_self">' . __('Settings') . '</a>',        
    ));
}      
function nxs_row_meta_nomem($data, $page){ if ( $page != NXSSNAP_BASENAME ) { return $data; } global $nxs_mLimit;
    return array_merge($data,array(                
...
if (!function_exists("nxs_getURL")){ function nxs_getURL($options, $postID, $addURLParams='') { global $nxs_SNAP; $gOptions = $nxs_SNAP->nxs_options;
  ...
  if (empty($forceSURL)) $forceSURL = !empty($options['useSURL']); else $forceSURL = $forceSURL =='1'; if (!empty($options['suUName'])) $forceSURL = false; //## SU does not allow Shorteners
  if ($forceSURL) { $mysurl = ''; $mysurlArr = get_post_meta($postID, 'snap_MYSURL', true); if (!empty($mysurlArr)) { $t = $mysurlArr['t']; $mysurl = ($t==$options['urlToUse'] && $mysurlArr['r']==$gOptions['nxsURLShrtnr'])?$mysurlArr['s']:''; }
    if (!empty($mysurl)) $options['urlToUse'] = $mysurl; else { $t = $options['urlToUse'];  $options['urlToUse'] = nxs_mkShortURL($t, $postID); update_post_meta($postID, 'snap_MYSURL', array('s'=>$options['urlToUse'],'t'=>$t,'r'=>$gOptions['nxsURLShrtnr'])); 
...
//## NXS DB Tables
if (!function_exists('nxs_checkAddLogTable')){ function nxs_checkAddLogTable(){ global $wpdb; $charset_collate = 'DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci'; 
...
if (!function_exists("nxssnap_enqueue_scripts")) { function nxssnap_enqueue_scripts(){  if (function_exists('get_current_screen')) $screen = get_current_screen(); else return; // prr($screen->id);  prr($screen);    
  if( is_object($screen) && $screen->base == 'post' || stripos($screen->id,'NextScripts_SNAP')!==false || stripos($screen->id,'nxssnap')!==false || stripos($screen->id, '_page_nxs')!==false) {          
      global $nxs_SNAP, $pagenow;  $options = $nxs_SNAP->nxs_options; if (empty($options['fltrs'])) $options['fltrs'] = array(); if (empty($options['fltrs']['nxs_post_type'])) $options['fltrs']['nxs_post_type'] = array('post');      
      if ($screen->base =='post' && ((empty($options['fltrs']['nxs_ie_posttypes']) && !in_array($screen->post_type, $options['fltrs']['nxs_post_type'])) || (!empty($options['fltrs']['nxs_ie_posttypes']) && in_array($screen->post_type, $options['fltrs']['nxs_post_type'])))) return;         
      ...
      if( stripos($screen->id,'toplevel_page_nxssnap')!==false  || stripos($screen->id,'_page_nxssnap-settings')!==false || stripos($screen->id,'_page_nxssnap-reposter')!==false ) { wp_enqueue_script( 'jquery-ui-datepicker' );            
...
if (!function_exists("nxsFavFooter")) { function nxsFavFooter() { if( current_user_can('editor') || current_user_can('administrator') ) { echo '<div style="display: none;" id="nxsFavNotice"><div id="nxsFavNoticeCnt">Posting....</div></div>'; }}}
...
if (!function_exists("jsPostToSNAP")) { function jsPostToSNAP() { if (function_exists('get_current_screen')) $screen = get_current_screen(); else return;   
  ...
  jQuery(document).ready(function() {         
    ...
    // Hide UPG Helper to 2.0 notice.
    jQuery('.nxs-upg200-notice .notice-dismiss').on( 'click', function() {
        jQuery.post(ajaxurl,{action: 'nxs_snap_aj',"nxsact":"hide-nxs-upg200-notice", _wpnonce: jQuery('input#nxsSsPageWPN_wpnonce').val()}, function(j){

        }, "html")
    });

      /* ## [SM] New Connection Ajax Call */
      jQuery(".sm_newConn_drpdwnmenu li a").click(function(){  var cn = jQuery(this).data("cn"); jQuery('#smNewConn').html('...'); jQuery("#smConnEditLabel").html('');
          jQuery.post(ajaxurl,{action:'sm_aj', act:'gNt', nt:jQuery(this).data("nt"),'cid':jQuery(this).data("cid"), et_load_builder_modules:1, _wpnonce: jQuery('input#sm_wpnonce').val()}, function(j){
              // jQuery('#smNewConn').html('...'); jQuery("#smConnEditBodyCnt").html(j); jQuery('#smNewConn').html(jQuery('#smConnEdit').html()); jQuery("#smConnEditPopupLabel").html('Connection Settings - '+cn);
              jQuery('#smNewConn').html(j); jQuery("#smConnEditLabel").html(cn);
          }, "html");
...
if (!function_exists("nxs_adminInitFunc")) { function nxs_adminInitFunc(){ global $nxs_SNAP, $pagenow;  $options = $nxs_SNAP->nxs_options;  
  ...
  if (current_user_can("haveown_snap_accss") || current_user_can("see_snap_box") || current_user_can("manage_options")) { add_action('add_meta_boxes', array($nxs_SNAP, 'addCustomBoxes'));        
...
if (!function_exists("nxs_admin_footer_pp")) {  function nxs_admin_footer_pp() { ?><div id="nxsPPHolder" style="display:none;"><div id="nxs_gPopupCntWrap" style="min-height: 300px;"><div id="nxs_gPopupContent"></div></div></div><?php }}
...
if (!function_exists('nxs_getPostImage')){ function nxs_getPostImage($postID, $size='large', $def='') { $imgURL = '';  global $nxs_SNAP;  if (!isset($nxs_SNAP)) return; 
  $options = $nxs_SNAP->nxs_options; $options['sImg'] = (defined('NXSAPIVER') && NXSAPIVER == '2.15.11')?1:0; if (!isset($options['ogImgDef'])) $options['ogImgDef'] = '';
  ...
  if ($imgURL!='' &&  empty($options['imgNoCheck']) && nxs_chckRmImage($imgURL)==false) $imgURL = ''; if ($imgURL!='') return $imgURL;    
  //## Default  
  if (trim($imgURL)=='' && trim($def)=='') $imgURL = $options['ogImgDef']; if (substr($imgURL, 0,4)!='http') $imgURL = '';
...
if (!function_exists("nxs_snapLogPublishTo")) { function nxs_snapLogPublishTo( $new_status, $old_status, $post ) { clean_post_cache( $post->ID ); $uid = 0; if ($post->post_type=='nxs_filter' || $post->post_type=='nxs_qp') return;  
...
if (!function_exists('nxs_LogIt')){ function nxs_LogIt($type, $action, $ntName, $ntType='', $title='', $extInfo='', $flt='snap', $uid=0){     
...
//## Comments import
if (!function_exists("nxs_addToRI")) { function nxs_addToRI($postID) { global $nxs_SNAP;  if (!isset($nxs_SNAP)) return; $options = $nxs_SNAP->nxs_options;
  ...
  array_unshift($riPosts, $postID);  $riPosts = array_unique($riPosts); $riPosts = array_slice($riPosts, 0, $options['riHowManyPostsToTrack']); update_option('NS_SNriPosts', $riPosts, false);
}}
...
//## Format
if (!function_exists('nxs_makeURLParams')){ function nxs_makeURLParams($params) { $templ = html_entity_decode(nxs_getFromGlobalOpt('addURLParams')); if (empty($templ)) return false; 
    if (preg_match('/%NTNAME%/', $templ)) $templ = str_ireplace("%NTNAME%", urlencode(trim($params['NTNAME'])), $templ);
    if (preg_match('/%NTCODE%/', $templ)) $templ = str_ireplace("%NTCODE%", urlencode(trim($params['NTCODE'])), $templ);
    if (preg_match('/%ACCNAME%/', $templ)) $templ = str_ireplace("%ACCNAME%", urlencode(trim($params['ACCNAME'])), $templ);
    if (preg_match('/%POSTID%/', $templ)) $templ = str_ireplace("%POSTID%", urlencode(trim($params['POSTID'])), $templ);
    if (preg_match('/%POSTTITLE%/', $templ)) { $post = get_post($params['POSTID']); if (is_object($post)) {$postName = $post->post_title; $templ = str_ireplace("%POSTTITLE%", urlencode(trim($postName)), $templ);}}
    if (preg_match('/%SITENAME%/', $templ)) { $siteTitle = urlencode(trim(htmlspecialchars_decode(get_bloginfo('name'), ENT_QUOTES))); $templ = str_ireplace("%SITENAME%", $siteTitle, $templ); }
    return $templ;
}}
//## Settings Export
...
function nxs_ogtgCallback($content){ global $post, $nxs_SNAP;  
  if (stripos($content, 'og:title')!==false) $ogOut = "\r\n"; else {
    if (!isset($nxs_SNAP)) $options = get_option('NS_SNAutoPoster'); else $options = $nxs_SNAP->nxs_options;    $ogimgs = array();  $accs = $nxs_SNAP->nxs_accts;  //$content = json_encode($accs['fb']).$content;
...
if (!function_exists('nxs_doNewNPPost')){ function nxs_doNewNPPost($networks){ global $nxs_snapAvNts, $wpdb; $postResults = '';  $currTime = time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS );
  $qpid = sanitize_key($_POST['qpid']);
  if (!empty($_POST['ddt'])) { $ddt = strtotime(str_replace(',','',$_POST['ddt'])); $isSch = $ddt>$currTime;} else $isSch = false; 
  if (!empty($_POST['nxs_mqTest']) && $_POST['nxs_mqTest']=="\'") { $_POST['mText'] = stripslashes($_POST['mText']); $_POST['mTitle'] = stripslashes($_POST['mTitle']); } $pst = $_POST;
  $ttl = nsTrnc(!empty($_POST['mTitle'])?sanitize_text_field($_POST['mTitle']):sanitize_text_field($_POST['mText']), 200);
  if (empty($ttl)) $ttl = 'Quick post ['.date('F j, Y, g:i a', time()+( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ).']'; //## Format title for saving info ....
  { //###### Make it savable as option. Put If () here.
    //## Insert Post 
    $user_id = get_current_user_id();
    $my_post = array( 'post_title' => $ttl, 'post_content' => sanitize_text_field($_POST['mText']), 'post_status' => 'publish', 'post_author' => $user_id, 'post_type' => 'nxs_qp', 'post_category' => array(  ) );
    if (!empty($qpid)) { $my_post['ID'] = $qpid; wp_update_post( $my_post );} else $qpid = wp_insert_post( $my_post );
    //## Insert Post meta
    if (!empty($qpid)) {  $metaArrEx = get_post_meta($qpid, '_nxs_snap_data', true );
        $metaArr = array('posts'=>array(), 'postType'=>sanitize_text_field($_POST['mType']), 'imgURL'=>sanitize_text_field($_POST['mImg']), 'linkURL'=>sanitize_text_field($_POST['mLink']));
        if (!empty($metaArrEx['posts'])) $metaArr['posts'] = $metaArrEx['posts']; update_post_meta($qpid, '_nxs_snap_data', $metaArr);
    }
  }  
...
if (!function_exists('nxs_postFromForm')){ function nxs_postFromForm($post, $networks, $isSilent=false){ global $nxs_snapAvNts; $postResults = '';
    if (!empty($post['mNts']) && is_array($post['mNts'])) {
      nxs_addToLogN('S', '-=== New Qiuck Post ===-', 'Form', count($post['mNts']).' Networks', sanitize_text_field(print_r($post['mNts'], true))); //.'<br/>|<br/><pre>'.print_r($post, true).'</pre>');
      ...
      if (!empty($post['mImg']) && substr($post['mImg'], 0, 4)=='http') $message['imageURL'] = sanitize_text_field($post['mImg']); $nts = array();  $postResultsArr = array('date'=> time(), 'errors'=>0, 'ok'=>0, 'data'=>array());
          
      foreach ($post['mNts'] as $ntC){ $ntA = explode('--',$ntC); $ntOpts = $networks[$ntA[0]][$ntA[1]]; $nts[] = $ntA[0].$ntA[1]; //  nxs_addToLogN('L', 'IN', $logNT, 'Go ', print_r($ntA, true));
        if (!empty($ntOpts) && is_array($ntOpts)) { $logNT = $ntA[0];  $clName = 'nxs_class_SNAP_'.strtoupper($logNT);                  
          ...
          $message['pText'] = nxs_doSpin(sanitize_text_field($post['mText'])); $message['pTitle'] = nxs_doSpin(sanitize_text_field($post['mTitle']));
          $ntOpts['postType'] = sanitize_text_field($post['mType']); $ntToPost = new $clName(); $ret = $ntToPost->doPostToNT($ntOpts, $message);   //   nxs_addToLogN('L', 'OUT', $logNT, 'Ex ', print_r($ntA, true));
          if (!is_array($ret) || empty($ret['isPosted']) || $ret['isPosted']!='1') { //## Error 
             ...
             if (!empty($ret['postURL'])) $extInfo = '<a href="'.esc_url($ret['postURL']).'" target="_blank">Post Link</a>'; //$extInfo .= ' | '.print_r($message, true).' | '.print_r($ntOpts, true);
    ...
    if (!empty($post['qpid'])) { $quid = sanitize_key($post['qpid']);
        $metaArr = get_post_meta($quid, '_nxs_snap_data', true ); $metaArr['nts'] = $nts; $metaArr['posts'][] = $postResultsArr;  update_post_meta($quid, '_nxs_snap_data', $metaArr);
    }
...
if (!class_exists('nxs_snapPostResults')) { class nxs_snapPostResults { var $info = array();  var $summary = ''; var $details = '';     
    ...
    function createSummary($info){ 
      ...
      if (!empty($info['ok'])) $out .= $info['ok'].' '.__('accounts - OK', 'social-networks-auto-poster-facebook-twitter-g').'; ';
      if (!empty($info['errors'])) $out .= $info['errors'].' '.__('Errors', 'social-networks-auto-poster-facebook-twitter-g').'; ';      
      $this->summary .= $out.'<br/>';
    }
...
//## New Post Form
if (!function_exists('nxs_showNewPostForm')){ function nxs_showNewPostForm($networks, $air = true) { global $nxs_snapAvNts; ?> 
...
<input type="text" id="nxs_jj" name="nxs_jj" value="<?php echo date_i18n('d'); ?>" size="2" maxlength="2" autocomplete="off">, <input type="text" id="nxs_aa" name="nxs_aa" value="<?php echo date_i18n('Y'); ?>" size="4" maxlength="4" autocomplete="off"> @ <input type="text" id="nxs_hh" name="nxs_hh" value="<?php echo date_i18n('H'); ?>" size="2" maxlength="2" autocomplete="off"> : <input type="text" id="nxs_mn" name="nxs_mn" value="<?php echo date_i18n('i'); ?>" size="2" maxlength="2" autocomplete="off"></div>
      ...
      <?php if ($air) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<input id="nxsNPCloseBt" style="width: 70px;" onclick="jQuery.pgwModal('close');" class="bClose" type="button" value="Cancel"> <?php } ?>
      </div> 
...
if (!function_exists('nxs_showNetworksList')){ function nxs_showNetworksList ($networks, $selected='') {  global $nxs_snapAvNts; $out = ''; 
...
function nxs_wp_allow_comment($commentdata) { global $wpdb; extract($commentdata, EXTR_SKIP); 
	...
	$dupe = "SELECT comment_ID FROM {$wpdb->comments} WHERE comment_post_ID = %d AND comment_parent = %d AND comment_approved != 'trash' AND (comment_author = %s ";
	if ($comment_author_email)  $dupe .= "OR comment_author_email = %s "; else $dupe .= "%s";
	$dupe .= ") AND comment_content = %s LIMIT 1 ";
	if ($comment_author_email) {
		$sql = $wpdb->prepare($dupe, $comment_post_ID, $comment_parent, $comment_author, $comment_author_email, $comment_content);
	} else {
		$sql = $wpdb->prepare($dupe, $comment_post_ID, $comment_parent, $comment_author, '', $comment_content);
	}
	$dupeID = $wpdb->get_var($sql);

	if ( $dupeID ) { do_action( 'comment_duplicate_trigger', $commentdata ); return $dupeID; }
...
//#### #4
if (!function_exists('nxs_importComments')){ function nxs_importComments() {  global $nxs_SNAP; if (!isset($nxs_SNAP)) return; $networks = $nxs_SNAP->nxs_accts; $options = $nxs_SNAP->nxs_options;  
  ...
  //## Facebook
  if (!empty($networks['fb']) && is_array($networks['fb'])) foreach ($networks['fb'] as $ii=>$fbo) if ($fbo['riComments']=='1') {  $fbo['ii'] = $ii; $fbo['pType'] = 'aj';
  ...
  //## Twitter
  if (!empty($networks['tw']) && is_array($networks['tw'])) foreach ($networks['tw'] as $ii=>$fbo) if (!empty($fbo['riComments']) && $fbo['riComments']=='1') {  $fbo['ii'] = $ii; $fbo['pType'] = 'aj'; 
    foreach ($riPosts as $postID) {  
