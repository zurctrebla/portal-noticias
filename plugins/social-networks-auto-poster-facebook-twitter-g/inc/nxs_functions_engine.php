...
if (!function_exists("nxs_snapPublishTo")) { function nxs_snapPublishTo($postIDorObj, $aj=false) {  global $nxs_SNAP, $nxs_snapAvNts, $blog_id, $nxs_tpWMPU; $uid=0;
    if (!isset($nxs_SNAP)) return;
    ...
    if(is_object($postIDorObj)) { $postObj = $postIDorObj; $postID = $postObj->ID; } else { $postID = $postIDorObj; $postObj = get_post($postID); } unset($postIDorObj); $isPost = isset($NXS_POST["snapEdIT"]);


    global $wp;
    if (stripos($wp->request,'wp-json/wp/v2/posts/')!==false) {  $tp = get_option('_nxs_tp'); if (empty($tp)) $tp = array(); $tp[] = $postID; update_option( '_nxs_tp', $tp, false );
        nxs_LogIt('I', 'Guttenberg', '','', 'Pending Autopost from Guttenberg.', 'Waiting for Metadata ('.print_r($tp, true).') - Post ID:('.$postID.') - Current Post status - '.$postObj->post_status, 'snap', $uid ); return;
    }

    $postUser = $postObj->post_author; $isItUserWhoCan = (!user_can($postUser, 'manage_options' ) && user_can($postUser, 'haveown_snap_accss')); //if ($isItUserWhoCan) $uid = $postUser;
    if ($isItUserWhoCan) { $nxs_SNAP = new nxs_SNAP($postUser); $networks = $nxs_SNAP->nxs_acctsU; $uid = $postUser; global $nxs_uid; $nxs_uid = $uid; } else { $networks = $nxs_SNAP->nxs_accts; }

    $options = $nxs_SNAP->nxs_options; if (empty($options)) return;
    if ($postObj->post_status != 'publish') { sleep(5);  $postObj = get_post($postID);
        if ($postObj->post_status != 'publish') {  nxs_LogIt('I', 'Cancelled', '','', 'Autopost Cancelled', 'Post is not "Published" Right now - Post ID:('.$postID.') - Current Post status -'.$postObj->post_status, 'snap', $uid ); return; }
    }
    //## User Security (&& MU)
    if ($isPost && empty($options['skipSecurity']) && !current_user_can("make_snap_posts") && !current_user_can("haveown_snap_accss") && !current_user_can("manage_options")) { nxs_LogIt('I', 'Skipped', '','', 'Current user can\'t autopost - Post ID:('.$postID.')','','snap',$uid ); return; }
    if (empty($options['skipSecurity']) && !user_can( $postUser, "make_snap_posts" ) && !user_can( $postUser, "haveown_snap_accss" ) && !user_can( $postUser, "manage_options")){ nxs_LogIt('I', 'Skipped', '', '', 'User ID '.$postUser.' can\'t autopost (please see <a target="_blank" href="https://www.nextscripts.com/support-faq/#a17">FAQ #1.7</a> for more info/solution)  - Post ID:('.$postID.')','','snap',$uid ); return; }
    //## Post
    if ($isPost) $nxs_SNAP->NS_SNAP_SavePostMetaTags($postID);
    ...
    $rMsg = nxs_snapCheckFilters($options, $postObj); if ($rMsg!==false) { nxs_LogIt('I', 'Skipped', '','', 'Filter(Global) - Excluded - Post ID:('.$postID.')',$rMsg,'snap',$uid ); return; }
    ...
    $snap_isAutoPosted = get_post_meta($postID, 'snap_isAutoPosted', true); if (!empty($snap_isAutoPosted)) { $snap_isAutoPosted = ($snap_isAutoPosted>1)?'(on '.date_i18n('Y-m-d H:i:s',$snap_isAutoPosted).')':'';
        nxs_LogIt('W', 'Skipped', '','', 'Already Autoposted ', $snap_isAutoPosted.' - Post ID:('.$postID.')' ,'snap',$uid); return;
    } $snap_isEdIT = get_post_meta($postID, 'snapEdIT', true);     //     var_dump($snap_isEdIT); 

    if (function_exists('nxs_v4doSMAS2')) { nxs_v4doSMAS2($postObj, $NXS_POST, $publtype,$aj); return; } else { #################################### !!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        foreach ($nxs_snapAvNts as $avNt) {
            if (!empty($networks[$avNt['lcode']]) && count($networks[$avNt['lcode']])>0) { $clName = 'nxs_snapClass'.$avNt['code']; $ntClInst = new $clName(); $publTempType = '';
                ...
                if ($optMt['do']=='1') { $optMt['ii'] = 0;
                    ...
                    if ($publtype=='S' || $publTempType=='S') { $publTempType = ''; if (isset($optMt['nHrs']) && isset($optMt['nMin']) && ($optMt['nHrs']>0 || $optMt['nMin']>0) ) { $delay = $optMt['nMin']*60+$optMt['nHrs']*3600;
                ...
                } else { nxs_LogIt('GR', 'Skipped', $avNt['name'].' ('.$optMt['nName'].')','', '-=[Unchecked Account]=-', 'PostID: '.$postID,'snap',$uid ); }
            }
        }
    }
    if (isset($isS) && $isS) restore_current_blog();
}}
if (!function_exists("nxs_noSing")) { function nxs_noSing( &$obj ) { $obj->is_singular = false; $obj->is_single = false; return $obj; }}

//## Functions
if (!function_exists('nxs_getFromGlobalOpt')){ function nxs_getFromGlobalOpt($optName) { global $nxs_SNAP;
    if (!isset($nxs_SNAP)) { if (class_exists('nxs_SNAP')) $nxs_SNAP = new nxs_SNAP(); else return ''; } $gOptions = $nxs_SNAP->nxs_options;  if (!empty($gOptions[$optName])) return $gOptions[$optName]; else return '';
}}
...
if (!function_exists("nxs_recountQueryTimes")) { function nxs_recountQueryTimes($force=false){ global $wpdb, $nxs_SNAP; if (!isset($nxs_SNAP)) return; $options = $nxs_SNAP->nxs_options; $currTime = nxs_getCurrTime();
    ...
    if (count($quPosts)>0) { $pstEvrySec = $options['quDays']*86400+$options['quHrs']*3600+$options['quMins']*60; $rndSec = $options['quLimitRndMins']*60; $ttr = time(); //$ttr = strtotime('2050-10-15 10:10:10'); 
        ...
        foreach ($quPosts as $row){ $id = $row['id'];
            if (!empty($row['postid'])) { $post = get_post($row['postid']); if (empty($post)) { $wpdb->delete($wpdb->prefix."nxs_query", array('id' => $id)); continue; } }
    ...
    if (count($quPosts)>0) foreach ($quPosts as $row){ $id = $row['id'];  if ($force || $row['timetorun'] > date_i18n('Y-m-d H:i:s', $currTime-600)) {
        ...
        if (!empty($rpstrOpts) && is_array($rpstrOpts)) { $rpstrOpts['rpstNxTime'] = nxs_getNextPostTime( $rpstrOpts['rpstDays'], $rpstrOpts['rpstHrs'], $rpstrOpts['rpstMins'],$rpstrOpts['rpstRndMins']);
            $dbItem = array('timetorun'=> date_i18n('Y-m-d H:i:s',$rpstrOpts['rpstNxTime']));  $whItem = array('id'=>$id); $wpdb->update( $wpdb->prefix . "nxs_query", $dbItem, $whItem);
            nxs_Filters::save_meta($id, 'nxs_rpstr', $rpstrOpts );
        }
    }}
}}

if (!function_exists("nxs_pushPostToNT")){function nxs_pushPostToNT($ntCode, $ii, $postID, $type='a'){ $postObj = get_post($postID); $nt = strtolower($ntCode); $ntU = strtoupper($ntCode);
    $postUser = $postObj->post_author; $isItUserWhoCan = (!user_can($postUser, 'manage_options' ) && user_can($postUser, 'haveown_snap_accss')); //if ($isItUserWhoCan) $uid = $postUser;
    if ($isItUserWhoCan) { $nxs_SNAP = new nxs_SNAP($postUser); $networks = $nxs_SNAP->nxs_acctsU; $uid = $postUser; global $nxs_uid; $nxs_uid = $uid; } else { global $nxs_SNAP; if (!isset($nxs_SNAP)) return; $networks = $nxs_SNAP->nxs_accts; }
    $optNt = maybe_unserialize(get_post_meta($postID, 'snap'.$ntU, true));
    $clName = 'nxs_snapClass'.$ntU; if (class_exists($clName)) { $cl = new $clName();
        if (empty($optNt) || !is_array($optNt)) $optNt = $networks[$nt][$ii]; else $optNt = $cl->adjMetaOpt($networks[$nt][$ii],$optNt[$ii]);
        $optNt['pType'] = 'aj';$cl->nt[$ii] = $optNt; if ($type=='r') $cl->isRepost=true; return $cl->publishWP($ii, $postID);
    }
}}
...
if (!function_exists("nxs_getNextPostTime")){ function nxs_getNextPostTime($d,$h,$m,$r=0){ $currTime = nxs_getCurrTime(); $sec = $d*86400+$h*3600+$m*60; $rndSecs = isset($r)?$r*60:0; $rndTime = rand(0-$rndSecs, $rndSecs); return $currTime + $sec + $rndTime; }}

//## New Scheduled Engine Functions
//## Main Cron Engine Function.
if (!function_exists("nxs_checkQuery")){ function nxs_checkQuery(){ set_time_limit(0); $arrOut = array(); nxs_cron_check();
    if (empty($_SERVER["HTTP_USER_AGENT"])) $_SERVER["HTTP_USER_AGENT"] = '?'; $tmCorr = get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ;  $isDebug = current_user_can('administrator') && !empty($_GET['dbg']);  //$isDebug = true;
    ...
    if (!empty($options['riActive']) && $options['riActive']=='1') { $howOften = !empty($options['riHowOften'])?$options['riHowOften']:15; $lastCommentsCheck = get_option('nxs_cronLastCommentsCheck'); //$howOften = 3;
        if (!empty($lastCommentsCheck)) { if ($lastCommentsCheck+($howOften*60)<$currTime) {  update_option('nxs_cronLastCommentsCheck', $currTime, false); nxs_importComments();  update_option('nxs_cronLastCommentsCheck', $currTime, false); } }
        else update_option('nxs_cronLastCommentsCheck', $currTime, false);
    }
...
// Get 20 tasks with prepared query
        ...
        foreach ($quPosts as $row){ $id = $row['id']; if (!empty($row['postid'])) $postID = $row['postid']; else $postID = ''; //prr($row); prr($row['type']);
            switch ( $row['type'] ) {
                case 'Q':  //## Post from Query (to All Networks)
                    nxs_LogIt('BG', 'Query', '','', '--===#### POST REQUEST FROM QUERY - Post ID:('.$postID.') ####===',print_r($row, true),'snap',$row['uid']); nxs_snapPublishTo($postID, true); $wpdb->delete($wpdb->prefix . "nxs_query", array('id'=>$id));
                    if ($options['nxsOverLimit']=='D') { $dateC = date("d", strtotime($row['datecreated'])); $dayN = date("d", strtotime($row['timetorun']));
                        if ($dayN!=$dateC) { $wpdb->delete($wpdb->prefix . "nxs_query", array('type'=>'Q'));
                            nxs_LogIt('BG', 'Query', '','', '--===#### QUERY CLEARED ####===-- OVERLIMIT: '.$options['nxsOverLimit'], date("d", strtotime($row['datecreated'])).' != '.date("d", strtotime($row['datecreated']),'snap',$row['uid']),'snap',$row['uid']);
                        }
                    }
                    break;
                case 'S': //## Post shedulled post. (to Individual Network)
                    $res = nxs_pushPostToNT($row['nttype'], $row['refid'], $postID); if ($res=='200') $arrOut[] = 'All OK - ID: '.$id.' ('.$row['nttype'].'|'.$row['refid'].'|'.$postID.')';
                ...
                case 'R': //## Reposter is active - check if it's time to re-post.
                    ...
                    if (empty($rpstrOpts['rpstOn'])) { $wpdb->delete( $wpdb->prefix . "nxs_query", array( 'postid' => $row['postid'] ) );  nxs_LogIt('DBG', 'Ghosted Reposter [Deleted]', 'RP','','Reposter ID:'.$row['postid'].' ', '-=[ '.print_r($rpstrOpts, true).' ]=-'); break; }
                    //##
                    ...
                    if (!empty($fltrOpts)) { $fltrOpts['posts_per_page'] = '1';
                        ...
                        if (isset($rpstrOpts['rpstBtwHrsType']) && $rpstrOpts['rpstBtwHrsType']=='D'){
                            ...
                            if (is_array($rpstBtwDays) && count($rpstBtwDays)>0) { $currDay = (int)date('w'); if ($currDay==0) $currDay = 7; if (!(in_array($currDay, $rpstBtwDays))) {
                                nxs_LogIt('I', 'Reposter: Skipped - Excluded Day - '.date_i18n('D'), 'Reposter ID:'.$row['postid'], '', '-=[ - ]=-', print_r($rpstrOpts, true)); break;
                            }}
                            //## Check Hours
                            ...
                            if ($rpstBtwHrsT>0) { $currHour = (int)date_i18n('H', $currTime);
                                if ( !( ($rpstBtwHrsF<$rpstBtwHrsT && $currHour<$rpstBtwHrsT && $currHour>=$rpstBtwHrsF) || ($rpstBtwHrsF>$rpstBtwHrsT && $currHour<$rpstBtwHrsF && $currHour>=$rpstBtwHrsT) )) {
                                    nxs_LogIt('I', 'Reposter: Skipped - Excluded Hour - '.$currHour, 'Reposter ID:'.$row['postid'], '', '-=[ - ]=-', print_r($rpstrOpts, true)); break;
                                }}
                        }

                        //## Type of Post New-to-Old, Old-to-New, Random
                        if (empty($rpstrOpts['rpstType']) || $rpstrOpts['rpstType']=='2') { $fltrOpts['orderby']='post_date'; $fltrOpts['order'] = 'ASC'; }
                        elseif ($rpstrOpts['rpstType']=='3') { $fltrOpts['orderby']='post_date'; $fltrOpts['order'] = 'DESC'; } elseif ($rpstrOpts['rpstType']=='1') { $fltrOpts['orderby']='rand'; $rpstrOpts['rpstStop']='W'; } //else
                        //## Apply filters
                        nxs_removeAllWPQueryFilters(); $ids = get_posts_ids_by_filter($fltrOpts); $pCnt = count($ids);
                        //## Do post to all specified networks
                        if (!empty($ids[0])) { nxs_LogIt('I', 'Reposter [Got Post ID]', 'Reposter ID:'.$row['postid'].' (Post ID: '.$ids[0].')', '', '', '');
                            foreach ($fltrOpts['nxs_NPNts'] as $ntCode=>$ntts) { foreach ($ntts as $iis=>$accs) { $res = nxs_pushPostToNT($ntCode, $iis,  $ids[0], 'r'); } }
                            update_post_meta($ids[0], 'snap_isAutoPosted', '1'); update_post_meta($ids[0], 'snap_isRpstd'.$row['postid'], time());
                            nxs_LogIt('I', 'Reposter [After the Post]', 'Reposter ID:'.$row['postid'].' (Post ID: '.$ids[0].')', '', '-=[ ]=-', print_r($rpstrOpts, true));
                            $rpstrOpts['lastID'] = $ids[0];
                            //## Add to Stats
                            $stats = maybe_unserialize(get_post_meta( $postID, 'nxs_rpstr_stats', true )); if (empty($stats['posted'])) $stats['posted'] = 0; $stats['posted']++; $stats['pstdLst'][] = $ids[0]; nxs_Filters::save_meta($row['postid'], 'nxs_rpstr_stats', $stats );
                            //## When finished: Repeat N times  - Stop it
                            if ($rpstrOpts['rpstStop']=='N') {  $n = $rpstrOpts['rpstStopRpt'];  if ($stats['posted']>=$n) {$rpstrOpts['rpstOn'] = 0; $wpdb->delete( $wpdb->prefix . "nxs_query", array( 'postid' => $row['postid'] ) );  $rpstrOpts['rpstOn'] = 'F';
                                nxs_LogIt('INF', 'Reposter Finished: Set to repeat '.$n.' times', 'RP','','Reposter ID:'.$row['postid'].' ', '-=[ '.print_r($rpstrOpts, true).' ]=-'); nxs_Filters::save_meta($row['postid'], 'nxs_rpstr', $rpstrOpts );  break;
                            }}
                        } else {
                            if ($isDebug) echo "END of the LINE!";
                            //## Turn Reposting off
                        ...
                        if ( $rpstrOpts['rpstOn'] =='1' && (empty($rpstrOpts['rpstTimes']) || $rpstrOpts['rpstTimes']=='A')) $rpstrOpts['rpstNxTime'] = nxs_getNextPostTime( $rpstrOpts['rpstDays'], $rpstrOpts['rpstHrs'], $rpstrOpts['rpstMins'],$rpstrOpts['rpstRndMins']); else {
                            global $nxs_cTime; $nxs_cTime = time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS );     nxs_LogIt('I', 'Reposter Check', 'Reposter ID:'.$row['postid'].' (CNT: '.$pCnt.')', '', '-=[ '.date('Y-m-d H:i:s', $nxs_cTime).' ]=-');
                            if (isset($rpstrOpts['rpstCustTD']) && is_array($rpstrOpts['rpstCustTD'])) {
                                $rpstrOpts['rpstCustTD'] = array_filter($rpstrOpts['rpstCustTD'], create_function('$value', 'global $nxs_cTime; return !empty($value) && strtotime($value)>$nxs_cTime;'));  sort($rpstrOpts['rpstCustTD']);
                            } else $rpstrOpts['rpstCustTD'] = array(); if (!empty($rpstrOpts['rpstCustTD'][0])) $rpstrOpts['rpstNxTime'] = strtotime($rpstrOpts['rpstCustTD'][0]); else {  $rpstrOpts['rpstNxTime'] = ''; $rpstrOpts['rpstOn'] = 0; }
                        }
                        nxs_Filters::save_meta($row['postid'], 'nxs_rpstr', $rpstrOpts );

                        $whItem = array('id'=>$id); if (!empty( $rpstrOpts['rpstNxTime'])) { $dbItem = array('timetorun'=> date_i18n('Y-m-d H:i:s',$rpstrOpts['rpstNxTime'])); $wpdb->update( $wpdb->prefix . "nxs_query", $dbItem, $whItem); } else $wpdb->delete( $wpdb->prefix . "nxs_query", $whItem);
                    }
                    break;
    ...
    } else update_option('nxs_contCron',false);
}}
