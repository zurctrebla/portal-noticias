...
class nxs_Filters {
    ...
    public static function save_filter( $post_id ) { 
      ...
      if ( get_post_type( $post_id ) === 'nxs_filter' ) {        
         ...
         if (!empty($pvData['nxs_cats_names'])) { $toIns = array(); $nCats = array(); 
         ...
         if (!empty($pvData['nxs_tags_names'])) { $nTags = array(); $toIns = array(); 
            ...
            $setToSave = array(); $pval = $pvData;
            
            //## Unset supplemental fields.
            if (isset($pvData['nxs_term_children']) && empty($pvData['nxs_term_names'])) unset($pvData['nxs_term_children']);
            if (isset($pvData['nxs_term_operator']) && empty($pvData['nxs_term_names'])) unset($pvData['nxs_term_operator']);
            if (isset($pvData['nxs_term_relation']) && empty($pvData['nxs_term_names'])) unset($pvData['nxs_term_relation']);
            // if (isset($pvData['nxs_count_term_compares']) && empty($pvData['nxs_term_names'])) unset($pvData['nxs_count_term_compares']);
            
            //## Post_Meta
            if (!empty($pval['nxs_count_meta_compares']))$setToSave['nxs_count_meta_compares'] = $pval['nxs_count_meta_compares'];      
            ...
            if (!empty($pval['nxs_count_meta_compares']) && (int)$pval['nxs_count_meta_compares']>1) for( $jj = 2; $jj <= $pval['nxs_count_meta_compares']; $jj++ ) { 
               if (!empty($pval['nxs_meta_key_'.$jj]) && $pval['nxs_meta_key_'.$jj]!='snap_isAutoPosted' && $pval['nxs_meta_key']!='snap_isRpstd'.$post_id) { $pm = array(); $jjj++;
            ...
            if (isset($pvData['nxs_rpstr']['rpstOnlyPUP'])) { $pm = array(); $jjj++; $pm['operator'] = 'NOT EXISTS'; $pm['key'] = 'snap_isAutoPosted'; $pm['value'][] = ''; $pm['relation'] = 'AND';  $setToSave['post_meta'][] = $pm; }            
            if (isset($pvData['nxs_rpstr']['rpstOnlyUniq']) || (isset($pvData['nxs_rpstr']['rpstType']) && $pvData['nxs_rpstr']['rpstType']!='1') ) { 
                $pm = array(); $jjj++; $pm['operator'] = 'NOT EXISTS'; $pm['key'] = 'snap_isRpstd'.$post_id; $pm['value'][] = ''; $pm['relation'] = 'AND';  $setToSave['post_meta'][] = $pm; 
            }            
            
            $setToSave['nxs_count_meta_compares'] = !empty($setToSave['post_meta'])?count($setToSave['post_meta']):0;
            
            if (isset($pvData['nxs_count_date_periods']) && empty($pvData['nxs_end_period']) && empty($pvData['nxs_starting_period'])) unset($pvData['nxs_count_date_periods']);
            if (isset($pvData['nxs_inclusive']) && empty($pvData['nxs_end_period']) && empty($pvData['nxs_starting_period'])) unset($pvData['nxs_inclusive']);
            
            //## If Taxonomy name is set and value is not - remove it 
            if (isset($pvData['nxs_tax_names']) && empty($pvData['nxs_term_names'])) unset($pvData['nxs_tax_names']);
            
            if (!empty($pvData['nxs_term_names']) && !empty($pvData['nxs_tax_names']) && is_array($pvData['nxs_term_names']) ) { $outT = array();
                foreach ($pvData['nxs_term_names'] as $g) { $term = get_term( $g, $pvData['nxs_tax_names'] ); if (!is_object($term)) { $t = wp_insert_term( $g, $pvData['nxs_tax_names']);  $outT[] = $t['term_id']; } else  $outT[] = $g; }
            ...
            if( $count_term_compares > 1 ) {
                for( $j = 2; $j <= $count_term_compares; $j++ ) {
                    ...
                    if (!empty($pvData['nxs_term_names_' .$j]) && !empty($pvData['nxs_tax_names_' .$j]) && is_array($pvData['nxs_term_names_' .$j]) ) { $outT = array();
                       foreach ($pvData['nxs_term_names_' .$j] as $g) { $term = get_term( $g, $pvData['nxs_tax_names_' .$j] ); if (!is_object($term)) { $t = wp_insert_term( $g, $pvData['nxs_tax_names_' .$j]); $outT[] = $t['term_id']; } else  $outT[] = $g; }
            ...
            if ($count_date_abs_periods > 1) for( $jj = 2; $jj <= $count_date_abs_periods; $jj++ ) { 
               if (!empty($pvData['nxs_start_abs_period_'.$jj]) || !empty($pvData['nxs_end_abs_period_'.$jj])) { $pm = array(); $jjj++;
            ...
            //prr($setToSave); 
            //##  
            $ntOpts = array(); if (!empty($pvData['nxs_NPNts'])) foreach ($pvData['nxs_NPNts'] as $ntC){ $ntA = explode('--',$ntC); $ntOpts[$ntA[0]][$ntA[1]] = 1; }$pvData['nxs_NPNts'] = $ntOpts; // prr($ntOpts);
            //## Save reposter 
            foreach( $settings as $setting ) { $full_setting_name = 'nxs_'. $setting;               
              if( !empty( $pvData[$full_setting_name] ) ) $setToSave[$full_setting_name] = $pvData[$full_setting_name];
            }             
    ...
    public static function save_schinfo( $post_id ) {  
        ...
        $optionsii = maybe_unserialize(get_post_meta( $post_id, 'nxs_rpstr', true )); $pval = $_POST['nxs_rpstr']; if (empty($optionsii)) $optionsii = array();
        ...
        if ($optionsii['rpstRndMins']>30) $optionsii['rpstRndMins'] = '30';
        if (empty($optionsii['rpstDays']) && empty($optionsii['rpstHrs']) && $optionsii['rpstRndMins']>($optionsii['rpstMins']/2) ) $optionsii['rpstRndMins'] = ceil(($optionsii['rpstMins']/2)-1);
        ...
        global $nxs_cTime;   $nxs_cTime = time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS );  
        if (isset($pval['rpstCustTD']) && is_array($pval['rpstCustTD'])) { 
            $pval['rpstCustTD'] = array_filter($pval['rpstCustTD'], create_function('$value', 'global $nxs_cTime; return !empty($value) && strtotime($value)>$nxs_cTime;'));  sort($pval['rpstCustTD']);  $optionsii['rpstCustTD'] = $pval['rpstCustTD']; 
        } else $optionsii['rpstCustTD'] = array(); 
        
        $isTD = (isset($pval['rpstTimes']) && $pval['rpstTimes']=='S' && !empty($pval['rpstCustTD']) && !empty($pval['rpstCustTD'][0])); if (empty($optionsii['rpstNxTime'])) $optionsii['rpstNxTime'] = 0;
        ...
        if (isset($pval['rpstTimes']))  $optionsii['rpstTimes'] = trim($pval['rpstTimes']);          
        ...
        if (isset($pval['rpstOnlyUniq']))  $optionsii['rpstOnlyUniq'] = trim($pval['rpstOnlyUniq']); else $optionsii['rpstOnlyUniq'] = 0;          
        ...
        if (empty($optionsii['lastID']) || !empty($_POST['resetStats'])) if (empty($optionsii['rpstType']) || $optionsii['rpstType']=='2') $optionsii['lastID'] = 0; elseif ($optionsii['rpstType']=='3') $optionsii['lastID'] = 999999999;        
        //  prr($pval);   prr($optionsii);        
        self::save_meta( $post_id, 'nxs_rpstr', $optionsii );  return $optionsii;
       
    }

    public static function getStats( $postID, $filter='', $rpstr='' ) { $stats = maybe_unserialize(get_post_meta( $postID, 'nxs_rpstr_stats', true )); $currTime = time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ); $phTxt = '';
        if (empty($filter)) $filter = maybe_unserialize(get_post_meta( $postID, 'nxs_rpstr_data', true )); if (empty($rpstr)) $rpstr = maybe_unserialize(get_post_meta( $postID, 'nxs_rpstr', true )); 
        if (empty($stats)) $stats = array('posts'=>0, 'posted'=>0, 'tbPosted'=>0, 'atPosted'=>0, 'fltText'=>'', 'statsText'=>'', 'nxTime'=>'', 'lpTime'=>'', 'lpID'=>'');
        if (!empty($filter)) { nxs_removeAllWPQueryFilters(); $ids = get_posts_ids_by_filter($filter); $pCnt = count($ids);  $stats['posts']=$pCnt; $stats['fltText'] = nxsAnalyzePostFilters($filter); } 
        ...
        $stats['lTime'] = !empty($rpstr['rpstLastShTime'])?date_i18n('Y-m-d H:i', $rpstr['rpstLastShTime']):__('Never','social-networks-auto-poster-facebook-twitter-g'); $stats['lpID'] = !empty($rpstr['rpstLastPostID'])?$rpstr['rpstLastPostID']:''; 
        if (!empty($stats['pstdLst'])) { $ph = array_slice( array_reverse($stats['pstdLst']), 0, 10); $phTxt = '<b>Posting History</b> (Last 10 records): <br/>'; foreach ($ph as $p) { $phTxt .=  $p.' - '.get_the_title($p).'<br/>'; }  }
        ...
        self::save_meta( $postID, 'nxs_rpstr_stats', $stats ); return $stats;
    }
    ...
    <b style="font-size: 15px;vertical-align: middle;"><?php _e('Get posts:', 'social-networks-auto-poster-facebook-twitter-g'); ?></b>
       <select id="riS<?php echo esc_attr($ii); ?>" name="nxs_rpstr[rpstType]" onchange="nxs_actDeActTurnOff(jQuery(this).attr('id'));">        
        ...
        <?php if (function_exists('nxs_doSMAS42')) nxs_doSMAS42($options); else {?> <option disabled="disabled">[Pro Only] Random</option> <?php }?>        
       </select> 
       <br/>
   ...
   <div class="nxs_tls_bdX">          
     <?php if (function_exists('ns_SMASV41')) {?> <input type="radio" name="nxs_rpstr[rpstTimes]" value="A" class="rpstrTimes" <?php if (empty($options['rpstTimes']) || $options['rpstTimes']=='A') echo 'checked="checked"'; ?> /><?php } ?><b><?php _e('Post every:', 'social-networks-auto-poster-facebook-twitter-g'); ?> </b>
     ...
     <div id="rpstPostEveryOptions" style="margin-left:20px;display:<?php echo (empty($options['rpstTimes']) || $options['rpstTimes']=='A')?'block':'none';?>;">
       ...
       <b><?php _e('When finished', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</b>&nbsp;  <?php if (function_exists('nxs_v4doSMAS412')) nxs_v4doSMAS412($options); else {?> <?php _e('Turn Reposting Off', 'social-networks-auto-poster-facebook-twitter-g') ?> 
        <input type="hidden" name="nxs_rpstr[rpstStop]" value="O"/> <?php }?>  
       </div> 
       <?php if (function_exists('nxs_v4doSMAS41')) nxs_v4doSMAS41($options); ?>
     </div>     
     
     <?php if (function_exists('nxs_v4doSMAS413')) nxs_v4doSMAS413($options); else { ?>
        ...
        foreach ($nxs_snapAvNts as $avNt) { $clName = 'nxs_snapClass'.$avNt['code']; $ntClInst = new $clName();
          if ( isset($networks[$avNt['lcode']]) && count($networks[$avNt['lcode']])>0) { ?>                
            ...
            <?php $ntOpts = $networks[$avNt['lcode']]; foreach ($ntOpts as $indx=>$pbo){                
              ...
              if (!empty($cp) && !empty($savedMeta)){ $svdNTs=!empty($savedMeta['nxs_NPNts'])?$savedMeta['nxs_NPNts']:''; $isCh=!empty($svdNTs)&&!empty($svdNTs[strtolower($avNt['code'])][$indx])&&$svdNTs[strtolower($avNt['code'])][$indx] == '1'; } 
    ...
    public static function print_posts_metabox( $current_post, $nt='', $ii='', $metaSettings='' ) { if (is_array($nt)) $nt = ''; $ntN = $nt.$ii;  
        ...
        //## Get saved Settinsg
        if (empty($metaSettings) && !empty( $current_post->ID)) $metaSettings = maybe_unserialize(get_post_meta( $current_post->ID, 'nxs_rpstr_data', true ));             //  prr($metaSettings, 'FLTRSZZ');
        /*
    ...
    public static function makeInputName($name, $nt='', $ii='') { $ntN = $nt.$ii; if ($nt=='exf') return 'src[exf][f]['.$name.']'; elseif (!empty($ntN)) return $nt.'['.$ii.']['.$name.']'; else return $name; }
    //## Sections
    public static function print_catsTags_section( $current_post, $metaSettings, $nt='', $ii='' ) { $ntN = $nt.$ii; if (empty($metaSettings)) $metaSettings = array();// ## Categories and Tags        
        ...
        self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $cat_names, 'nxs_cats_names'.$ntN, !empty($metaSettings['nxs_cats_names'])?$metaSettings['nxs_cats_names']:'', true, true, self::makeInputName('nxs_cats_names', $nt, $ii),'nxsSelItAjx', 'category' ); echo '</div>';
        ...
        self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $tags_names, 'nxs_tags_names'.$ntN, !empty($metaSettings['nxs_tags_names'])?$metaSettings['nxs_tags_names']:'', true, true, self::makeInputName('nxs_tags_names', $nt, $ii), 'nxsSelItAjxAdd', 'post_tag' ); echo '</div>';

        
        echo "</div>";
}
    public static function print_timeframe_section( $current_post, $metaSettings, $nt='', $ii='' ) {   // ## Dates
    ...
    public static function print_dates_section( $current_post, $metaSettings, $nt='', $ii='') { //## ABS DAtes
        ...
        $count_abs_periods = (!empty( $current_post->ID) && !empty($metaSettings['nxs_count_abs_periods']) )?$metaSettings['nxs_count_abs_periods']:'';  if( empty( $count_abs_periods ) ) $count_abs_periods = 1;                    
        echo '<div id="nxs_abstimeTopDiv'.$nt.$ii.'">';     
        
        for( $j = 1; $j <= $count_abs_periods; $j++ ) {  $postfix = $j == 1 ? '' : '_'. $j;  $rel = $j == 1 ? '' : 'nxs_abs_period_'. $j; $indx = $j-1; if ($j>1 && empty($metaSettings["absPeriods"][$indx])) continue;
    ...
    public static function print_types_section( $current_post, $metaSettings, $nt='', $ii='' ) {   $ntN = $nt.$ii; 
        ...
        */   
        if( !empty( self::$posts_types ) ) { 
            if (empty($current_post)&&empty($metaSettings)&&empty($nt)&&empty($ii)) $metaSettings = array('nxs_post_type' => array('0'=>'post')); 
            ...
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, self::$posts_types, 'nxs_post_type', !empty($metaSettings['nxs_post_type'])?$metaSettings['nxs_post_type']:'', false, true, self::makeInputName('nxs_post_type', $nt, $ii)); echo '</div>';
        }        
        if( !empty( $formats ) ) {
    ...
    public static function print_postsPages_section( $current_post, $metaSettings, $nt='', $ii='' ) {   $isVis = !empty($metaSettings['nxs_post_ids']) || !empty($metaSettings['nxs_post_ids']) || !empty($metaSettings['nxs_post_ids']);
    ...
    public static function print_postsPagesX_section( $current_post, $metaSettings, $nt='', $ii='' ) {   $isVis = !empty($metaSettings['nxs_name_post']) || !empty($metaSettings['nxs_name_page']) || !empty($metaSettings['nxs_name_parent']);
    // ## Exact Posts and Pages
       echo '<h4 onclick="jQuery(\'#nxs_sec_postsPages\').toggle();"; style="cursor:pointer; background-image: url(\''.NXS_PLURL.'img/icons/post24.png\');background-repeat: no-repeat; padding-top: 2px; padding-left: 28px; height:24px;">'. 
         __( 'Exact Posts and Pages', 'social-networks-auto-poster-facebook-twitter-g' ) .'&nbsp;&nbsp;&gt;&gt; </h4><div id="nxs_sec_postsPages" class="nxsLftPad" style="display:'.($isVis?'block':'none').';">';
    ...
    public static function print_meta_section( $current_post, $metaSettings, $nt='', $ii='' ) { if (empty($metaSettings)) $metaSettings = array(); if (empty($metaSettings['post_meta'])) $metaSettings['post_meta'] = array();
        //## "Repost ONLY previously unautoposted posts " checkbox - remoive from visible interface
        if (!empty($metaSettings['post_meta'])) foreach ($metaSettings['post_meta'] as $kk=>$vk) if ($vk['key']=='snap_isAutoPosted') unset($metaSettings['post_meta'][$kk]); 
        //## Show Post Meta section.
        $isVis = !empty($metaSettings['post_meta']); $ntN = $nt.$ii; 
        ...
        $count_compares = !empty($metaSettings['post_meta'])?count($metaSettings['post_meta']):1; $relation_options = array( 'AND', 'OR' );        
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;    $rel = $i == 1 ? '' : 'nxs_meta_compare_'. $i;  $jj = $i-1;   
          if (empty($metaSettings['post_meta'][$jj])) $metaSettings['post_meta'][$jj] = array();  if ($i>1 && empty($metaSettings['post_meta'][$jj]["key"])) continue;   
...
<input name="<?php echo self::makeInputName("nxs_meta_key$postfix", $nt, $ii); ?>" id="nxs<?php echo esc_attr($nt.$ii); ?>_meta_key<?php echo $postfix; ?>" value="<?php echo !empty($metaSettings['post_meta'][$jj]["key"])?$metaSettings['post_meta'][$jj]["key"]:'';?>"  style="font-weight: bold; color: #005800; border: 1px solid #ACACAC; width: 95%;"/>
    ...
    public static function print_taxonomies_section( $current_post, $metaSettings, $nt='', $ii='' ) { $isVis = !empty($metaSettings['nxs_term_names']); // prr($metaSettings); //  $metaSettings['nxs_count_term_compares'] = 1;
        //## Taxonomies         
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;    $rel = $i == 1 ? '' : 'nxs_term_compare_'. $i; if ($i>1 && empty($metaSettings["nxs_term_names$postfix"])) continue; 
            ...
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $taxs_names, "nxs".$nt.$ii."_tax_names$postfix", !empty($metaSettings["nxs_tax_names$postfix"])?$metaSettings["nxs_tax_names$postfix"]:'', true, false, self::makeInputName("nxs_tax_names$postfix", $nt, $ii), 'nxs_tax_names' );echo '</div>';
            
            echo '<div class="'. 'nxs_short_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Operator', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label><br/>';
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $compare_options, "nxs".$nt.$ii."_term_operator$postfix", !empty($metaSettings["nxs_term_operator$postfix"])?$metaSettings["nxs_term_operator$postfix"]:'', false, false, self::makeInputName("nxs_term_operator$postfix", $nt, $ii),'hui' ); echo '</div>';
            
            echo '<div class="'. 'nxs_short_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Child Terms', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>';
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $children_options, "nxs".$nt.$ii."_term_children$postfix", !empty($metaSettings["nxs_term_children$postfix"])?$metaSettings["nxs_term_children$postfix"]:'', true, false, self::makeInputName("nxs_term_children$postfix", $nt, $ii), 'hiu' ); echo '</div>';
    ...
    public static function print_lang_section( $current_post, $metaSettings, $nt='', $ii='' ) {  $isVis = !empty($metaSettings['nxs_langs']); $ntN = $nt.$ii;  $langs = array(); 
        // ## Language    
        ...
        //## PolyLang
        if (function_exists('pll_languages_list')) { $args = array('fields'=>'name'); $l1 = pll_languages_list(); $l2 = pll_languages_list($args); foreach ($l1 as $jj=>$val) $langs[$val] = $l2[$jj]; } 
        //## WPML
        elseif (function_exists('icl_get_languages')) { $l = icl_get_languages('skip_missing=0&link_empty_to=str'); foreach ($l as $lc=>$lObj) $langs[$lc] = !empty($lObj['translated_name'])?$lObj['translated_name']:$lObj['native_name']; } 
        //## qTranslateX 
        elseif (function_exists('qtrans_getSortedLanguages')) { $l = qtrans_getSortedLanguages();
          foreach ($l as $lc=>$lObj) $langs[$lc] = $lObj['translated_name'];         
        }  //prr($langs);
        ...
        if( !empty( $langs ) ) {            
            ...
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $langs, 'nxs_langs', !empty($metaSettings['nxs_langs'])?$metaSettings['nxs_langs']:'', true, true, self::makeInputName('nxs_langs', $nt, $ii) ); echo '</div>';
        }
        echo "</div>";
    ...
    public static function printSimpleSelect( $sID, $sName, $value, $optnsList, $sClass = 'nxsSelIt', $txnType='') { $ph = __('Please select from the list...', 'social-networks-auto-poster-facebook-twitter-g');
      ?> <select name="<?php echo $sName; ?>" id="<?php echo $sID; ?>" class="<?php echo $sClass; ?>" data-type="<?php echo $txnType; ?>" placeholder="<?php echo $ph; ?>">                 
      <?php foreach( $optnsList as $key => $optionName ) { $selected = $key==$value ? 'selected="selected"' : ''; echo '<option value="'. esc_attr( $key ) .'" '. $selected .'>'. esc_attr( $optionName ) .'</option>'; } ?>
      </select><?php
    }        
    ...
    public static function printSelect( $sID, $sName, $isMultiple, $optnsList, $values, $postID, $useKeyAsValue = false, $sClass = 'nxsSelIt', $txnType='') { $ph = __('Please select from the list...', 'social-networks-auto-poster-facebook-twitter-g');
      ...
      <?php
        foreach( $optnsList as $key => $optionName ) { $value = $useKeyAsValue ? $key : $optionName; $selected = self::search_value_in_filter_meta( $postID, $sName, $value, $values ) ? 'selected="selected"' : '';            
    ...
    public static function search_value_in_filter_meta( $post_id, $meta_name, $needle, $meta_value='' ) {        
        if (!empty($meta_value) && !is_array($meta_value) && $needle==$meta_value) return true;        
    ...
    public static function print_select( $post_id, $values, $selID_meta_name, $currVal='', $print_key = false, $multiple = true, $name='', $class = 'nxsSelIt', $txnType='' ) { $ph = __('Please select from the list...', 'social-networks-auto-poster-facebook-twitter-g');
      if (empty($name)) $name = $selID_meta_name;
...
if (!function_exists("nxs_snapCheckFilters")) { function nxs_snapCheckFilters($options, $postObj) { $postID = $postObj->ID; //  prr($options, 'FLT1');
  if (!empty($options['fltrsOn']) && !empty($options['fltrs']) && empty($options['fltrAfter'])) {  $options['fltrs']['nxs_postID'] = $postID; $options['fltrs']['fullreturn']='1'; //echo "|Pre FLT 2|"; 
    nxs_removeAllWPQueryFilters(); add_filter( 'pre_get_posts', 'nxs_noSing' ); $pfidRet = get_posts_ids_by_filter( $options['fltrs'] ); /* prr($pfidRet); */ $pfid = $pfidRet['p']; remove_filter( 'pre_get_posts', 'nxs_noSing' ); // echo "|W|"; prr($pfidRet); prr($pfid);
    if (empty($pfid) || empty($pfid[0]) || $pfid[0]!=$postID) { $msg = nxsAnalyzePostFilters($options['fltrs'], $postObj); 
      ...
      if (empty($msg)) return 'Filters Block';
...
//## Analyze Filters
if (!function_exists("nxsAnalyzePostFilters")) { function nxsAnalyzePostFilters($filter, $post='' ) { $out = ''; if (!empty($post) && !is_object($post)) $post = get_post($post); //prr($post); prr($filter); 
    //## Cats
    ...
    //## Post IDS
    if (!empty($filter['nxs_post_ids'])) { $fltT = '';       
      foreach ($filter['nxs_post_ids'] as $cctts) { $cInfo = get_post($cctts); $fltT .= $cInfo->post_title." [ID:".$cInfo->ID."] | "; }
    ...
    //## Time Frame
    if (!empty($filter['nxs_starting_period']) || !empty($filter['nxs_end_period'])) { $count_compares =  (int)$filter['nxs_count_date_periods']; 
      for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;     
        if (!empty($filter['nxs_starting_period'.$postfix]) && !empty($filter['nxs_end_period'.$postfix])) {  $out .= "<br/>\r\n".'&nbsp;&nbsp;&nbsp;&nbsp;Filter: Time (Autopost Only): Posts from: '.$filter['nxs_starting_period'.$postfix].' to  '.$filter['nxs_end_period'.$postfix];  
     ...
     //## ABS Time Frame
    if (!empty($filter['absPeriods'])) { $count_compares =  (int)$filter['nxs_count_abs_periods']; 
      for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;  $indx = $i-1;  if (empty($filter['absPeriods'][$indx])) continue; $arr = $filter['absPeriods'][$indx];
      
        if (!empty($arr['start']) && !empty($arr['end'])) {  $out .= "<br/>\r\n".'&nbsp;&nbsp;&nbsp;&nbsp;Filter: (Autopost Only): Posts newer than '.$arr['start'].' '.$arr['typeStart'].' and older than '.$arr['end'].' '.$arr['typeEnd'];  
    ...
    //## Meta
    if (!empty($filter['nxs_count_meta_compares'])) { $count_compares =  (int)$filter['nxs_count_meta_compares']; 
      for( $i = 0; $i < $count_compares; $i++ ) { 
        if (!empty($filter['post_meta'][$i]['key'])) { if (!empty($post)) { $val = get_post_meta( $post->ID, $filter['post_meta'][$i]['key']); if (empty($val)) $val = 'NULL'; } else $val = '';
...
if (!function_exists("nxs_check_dates")) { function nxs_check_dates($date){ if (strtotime($date)==false && strpos($date,'/')!==false) return (str_replace('/', '-', $date)); else return $date; }}

function get_posts_ids_by_filter( $filter ) { $args = array('fields' => 'ids'); //prr($filter);
    ...
    if( !empty( $filter['nxs_post_ids'] ) ) $args['post__in'] = $filter['nxs_post_ids'];
    ...
    if (function_exists('nxs_doSMAS7')) $args = nxs_doSMAS7($args, $filter); else {
        if (!empty($filter['post_meta']) && is_array($filter['post_meta']) ) { $meta_compares = array(); foreach ($filter['post_meta'] as $pm) if ($pm['key']=='snap_isAutoPosted' || stripos($pm['key'], 'snap_isRpstd')!==false) {  $new_compare = array(); 
             $new_compare['key'] = $pm['key']; $new_compare['compare'] = 'NOT EXISTS'; $new_compare['value'] = ''; $meta_compares[] = $new_compare;                
          } if (!empty($meta_compares)) $meta_compares['relation'] = 'AND'; $args['meta_query'] = $meta_compares;    
        }
    }       
    //## Post formats
    ...
    if( isset( $filter['nxs_count_date_periods'] ) && ( !empty( $filter['nxs_starting_period'] ) || !empty( $filter['nxs_end_period'] ) ) ) {        
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i > 1 ? '_'. $i : ''; $new_compare = array();            
            if( !empty( $filter["nxs_starting_period$postfix"] ) ) $new_compare['after'] = nxs_check_dates($filter["nxs_starting_period$postfix"]);
            if( !empty( $filter["nxs_end_period$postfix"] ) ) $new_compare['before'] = nxs_check_dates($filter["nxs_end_period$postfix"]);
    ...
    */
    if (!empty($filter['absPeriods'])) { $abs_date_compares = array(); $abs_count_compares = $filter['nxs_count_abs_periods'];
        for( $i = 1; $i <= $abs_count_compares; $i++ ) { $postfix = $i > 1 ? '_'. $i : ''; $new_abs_compare = array();  $indx = $i-1;  if (empty($filter['absPeriods'][$indx])) continue; $arr = $filter['absPeriods'][$indx];           
    ...
    //## WPML
    if (function_exists('wpml_get_language_information') && !empty($args['lang'])) $args['posts_per_page'] = -1;
    //############### GET Posts    
    $args['suppress_filters'] = true;  $postsRet = nxs_get_posts( $args ); $post_ids = $postsRet['p']; $qu = $postsRet['q']; 
    // $la = array('msg'=>'FLT Res X', 'extInfo'=>print_r($filter, true)." | ".print_r($post_ids, true));   nxsLogIt($la);
    
    //## WPML
    if (function_exists('wpml_get_language_information') && !empty($args['lang'])){ $pii = array(); 
        foreach ($post_ids as $pid) { $clang = apply_filters( 'wpml_post_language_details', NULL, $pid ); 
            if (in_array($clang['language_code'], $filter['nxs_langs'])) $pii[] = $pid; 
        } $post_ids = $pii;
    }   
