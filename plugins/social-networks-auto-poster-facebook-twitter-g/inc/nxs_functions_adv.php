...
if (!function_exists("nxs_snapAjax")) { function nxs_snapAjax() { check_ajax_referer('nxsSsPageWPN'); $arg = ''; nxs_Filters::init(true);
  ...
  if ($_POST['nxsact']=='setAllXS' && !empty($_POST['nid']) && !empty($_POST['uid'])) { $acnt = 0; $nid = explode('-',$_POST['nid']); $val = $networks[$nid[0]][$nid[2]][$nid[1]]; $uid = $networks[$nid[0]][$nid[2]][$_POST['uid']];      
	...
	if ($_POST['nxsact']=='hide-nxs-upg200-notice') {	update_option( 'nxs-upg200-notice-dismissed', 1 ); }
  ...
  if ($_POST['nxsact']=='deleteAllSNAPInfo') {
    ...
    } else echo '<script> setTimeout(function () { location.reload(1); }, 3000); </script>';
    _e('Done. All SNAP data has been removed. Please wait for the page to reload....', 'social-networks-auto-poster-facebook-twitter-g');
  } 
  
  if ($_POST['nxsact']=='accsFltToAll') { foreach ($networks as $ntl => $nta) foreach ($nta as $ii => $ntAcc) {
      $networks[$ntl][$ii]['fltrs']['nxs_post_type'] = array('post'); $networks[$ntl][$ii]['fltrsOn'] = '1';
...
//## List sort
if (!function_exists('nxsLstSort')){function nxsLstSort($a, $b) { if (empty($a['do'])) $a['do'] = 0; if (empty($b['do'])) $b['do'] = 0; if ($a['do']=='2') $a['do'] = 1;  if ($b['do']=='2') $b['do'] = 1; 
    if ($a['do'] == $b['do']) {if(empty($a['nName'])) $a['nName'] = 'z'; if(empty($b['nName'])) $b['nName'] = 'z'; return strcasecmp($a['nName'],$b['nName']); } return ($a['do'] > $b['do']) ? -1 : 1;    
}}
...
if (!function_exists("nsFormatMessage")) { function nsFormatMessage($msg, $postID, $addURLParams='', $lng='', $ntOpts=''){ global $ShownAds, $nxs_SNAP, $nxs_urlLen;
  ...
  $msg = nxs_doSpin($msg);
  if (stripos($msg, '%URL%')!==false) { if (!empty($ntOpts) && !empty($ntOpts['urlToUse'])) $url = $ntOpts['urlToUse']; else { $oo=array(); $oo = nxs_getURL($oo, $postID, $addURLParams); $url = $oo['urlToUse']; } 
  ...
  if (stripos($msg, '%ORID%')!==false) { $msg = str_ireplace("%ORID%", $postID, $msg); }
  if (stripos($msg, '%IMG%')!==false) { $imgURL = nxs_getPostImage($postID, !empty($ntOpts['wpImgSize'])?$ntOpts['wpImgSize']:'full'); $msg = str_ireplace("%IMG%", $imgURL, $msg); } 
  if (stripos($msg, '%TITLE%')!==false) { $title = nxs_doQTrans($post->post_title, $lng); $msg = str_ireplace("%TITLE%", $title, $msg); }                    
  ...
  if (stripos($msg, '%PANNOUNCE%')!==false) { $postContent = apply_filters('the_content', nxs_doQTrans($post->post_content, $lng));
    $postContent = strip_tags(strip_shortcodes(str_ireplace('<!--more-->', '#####!--more--!#####', str_ireplace("&lt;!--more--&gt;", '<!--more-->', $postContent))));
    if (stripos($postContent, '#####!--more--!#####')!==false) { $postContentEx = explode('#####!--more--!#####',$postContent); $postContent = $postContentEx[0]; }    
      else $postContent = nsTrnc($postContent, $options['anounTagLimit'], ' ', '');  $msg = str_ireplace("%PANNOUNCE%", $postContent, $msg);
  } 
  if (stripos($msg, '%ANNOUNCER%')!==false) { $postContent = nxs_doQTrans($post->post_content, $lng);     
    $postContent = strip_tags(strip_shortcodes(str_ireplace('<!--more-->', '#####!--more--!#####', str_ireplace("&lt;!--more--&gt;", '<!--more-->', $postContent))));
  ...
  if (stripos($msg, '%PANNOUNCER%')!==false) { $postContent = apply_filters('the_content', nxs_doQTrans($post->post_content, $lng));
    $postContent = strip_tags(strip_shortcodes(str_ireplace('<!--more-->', '#####!--more--!#####', str_ireplace("&lt;!--more--&gt;", '<!--more-->', $postContent))));
    if (stripos($postContent, '#####!--more--!#####')!==false) { $postContentEx = explode('#####!--more--!#####',$postContent); $postContent = $postContentEx[1]; }    
  ...
  if (stripos($msg, '%EXCERPT%')!==false) {      $exc = get_the_excerpt($post);
    if (!empty($exc)) $excerpt = strip_tags(strip_shortcodes(apply_filters('the_content', nxs_doQTrans($exc, $lng)))); 
  ...
  if (stripos($msg, '%RAWEXCERPT%')!==false) {    $exc = get_the_excerpt($post);
    if (!empty($exc)) $excerpt = strip_tags(strip_shortcodes(nxs_doQTrans($exc, $lng))); else $excerpt= nsTrnc(strip_tags(strip_shortcodes(nxs_doQTrans($post->post_content, $lng))), 300, " ", "..."); 
    $msg = str_ireplace("%RAWEXCERPT%", $excerpt, $msg);
  }
  if (stripos($msg, '%RAWEXCERPTHTML%')!==false) {  $exc = get_the_excerpt($post);
  ...
  if (stripos($msg, '%POSTFORMAT%')!==false) { $gg = get_post_format($postID);  $txt =  get_post_format_string($gg ? $gg : 'Post'); if (empty($txt)) $txt = 'Post'; $msg = str_ireplace("%POSTFORMAT%", $txt, $msg);}      
...
if (!function_exists("nxs_show_noLibWrn")) { function nxs_show_noLibWrn($msg){ ?> <div style="border: 2px solid darkred; padding: 25px 15px 15px 15px; margin: 3px; background-color: #fffaf0;"> 
            ...
            <div style="padding-bottom: 5px;"><a href="https://www.nextscripts.com/snap-api/">SNAP Premium API libraries package</a> adds autoposting to:</div> <span class="nxs_txtIcon nxs_ti_gp">Google My Business</span>, <span class="nxs_txtIcon nxs_ti_pn">Pinterest</span>, <span class="nxs_txtIcon nxs_ti_rd">Reddit</span>, <span class="nxs_txtIcon nxs_ti_bg">Blogger</span>,&nbsp;&nbsp;<span class="nxs_txtIcon nxs_ti_yt">YouTube</span>,&nbsp;&nbsp;<span class="nxs_txtIcon nxs_ti_fp">Flipboard</span>, <span class="nxs_txtIcon nxs_ti_li">LinkedIn Groups</span>, etc...<br><br>
...
//## Tests 
if (!function_exists("nxs_memCheck")) { function nxs_memCheck() { $mLimit = (int) ini_get('memory_limit'); $mLimit = empty($mLimit) ? __('N/A') :$mLimit . __(' MByte');
