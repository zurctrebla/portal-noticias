...
if ( ( td_util::get_option('tds_disable_comments_sidewide' ) == '' ) && post_type_supports( get_post_type(), 'comments' ) ) {
    ...
    <div class="comments" id="comments">
                    ...
                    array(
                        'callback' => function ( $comment, $args, $depth ) {
                            ...
                            <li class="comment <?php echo esc_attr( $td_isPingTrackbackClass ) ?>" id="comment-<?php comment_ID() ?>">
                                <article>
                                    <footer>
                                        <?php echo get_avatar( $td_comment_auth_email, 50, '', $comment->comment_author); ?>
        ...
        if (!function_exists('td_safe_sprintf_attr')) {
            /**
             * Safely formats a string with one %s, correcting common translation issues.
             */
            function td_safe_sprintf_attr($format, $value) {
                // Normalize broken placeholders: "% s" -> "%s"
                $format = preg_replace('/%\s+([a-zA-Z])/i', '%$1', $format);

                // Remove invalid sequences like "%."
                $format = preg_replace('/%[^a-zA-Z%]/', '', $format);

                // Count valid format placeholders
                preg_match_all('/(?<!%)%[a-z]/i', $format, $matches);

                // Use sprintf only if exactly one %s
                if (count($matches[0]) === 1 && in_array('%s', $matches[0], true)) {
                    return esc_attr(sprintf($format, $value));
                }
        ...
        $defaults['logged_in_as'] = '<p class="logged-in-as">' . sprintf(
            /* 1: edit user link, 2: accessibility text, 3: user name, 4: logout URL */
                ...
                get_edit_user_link(),
                /* %s: user name */
                td_safe_sprintf_attr( __td( 'Logged in as %s. Edit your profile.' , TD_THEME_NAME), $user_identity ),
                $user_identity,
