...
// ajax: categories/tags load more
add_action('wp_ajax_nopriv_td_ct_load_more', array('td_ajax', 'on_ct_load_more'));
add_action('wp_ajax_td_ct_load_more',        array('td_ajax', 'on_ct_load_more'));
...
add_filter( 'tiny_mce_before_init', 'tagdiv_editor_sidebar_style' );
function tagdiv_editor_sidebar_style( $mce_init ) {
	if ( tagdiv_options::get( 'tds_editor_show_sidebar' ) === 'yes' ) {
		$img_url = esc_url( get_stylesheet_directory_uri() . '/images/editor-style/grid-sidebar-desktop.gif' );
		$sidebar_css = "html .mceContentBody { max-width: 696px !important; background: #fff url('" . $img_url . "') repeat-y left top !important; } html .mceContentBody:after { content: 'SIDEBAR' !important; color: #ccc !important; position: absolute !important; right: -50px !important; top: 0 !important; display: block !important; width: 10px !important; word-break: break-all !important; }";
		if ( isset( $mce_init['content_style'] ) ) {
			$mce_init['content_style'] .= ' ' . $sidebar_css;
		} else {
			$mce_init['content_style'] = $sidebar_css;
		}
	}
...
function td_bottom_code() {
    ...
    <!--
        ...
        Theme: ' . TD_THEME_NAME . ' by tagDiv.com 2026
...
function td_gallery_shortcode($output = '', $atts = [], $content = false) {
	...
	$page_template_slug = !empty($post) ? get_page_template_slug($post->ID) : '';
...
function on_widget_display_callback($currentWidgetInstanceSettings, $currentWidgetInstance, $widgetArgs) {
    ...
    // If another plugin (e.g. WPML) short-circuits the widget, respect it.
    if ( ! is_array($currentWidgetInstanceSettings) ) {
    ...
    if ( isset($widgetArgs['widget_id']) && strpos($widgetArgs['widget_id'], 'td_block') !== 0 ) {
        ...
        $block_title_class = ($global_block_template_id === 'td_block_template_1') ? 'block-title' : 'td-block-title';
...
// Add keyboard accesibility class on body when enabled
add_filter('body_class', function ($classes) {
    $enabled = td_util::get_option('tds_keyboard_accessibility');
    if ($enabled === 'enabled') {
        $classes[] = 'a11y-enabled';
    }
    return $classes;
});
...
/**
 * Expose td_post_video (Featured Video) via the core WP REST API (wp/v2/posts).
 * Allows: meta.td_post_video.td_video
 *
 * Controlled by theme panel option:
 *   tds_rest_featured_video = 'enabled' (disabled by default)
 */
if ( ! function_exists( 'td_register_featured_video_meta_rest' ) ) {

    function td_register_featured_video_meta_rest() {

        // Feature gate (disabled by default).
        if ( td_util::get_option( 'tds_rest_featured_video' ) !== 'enabled' ) {
            return;
        }

        // If already registered with REST exposure, do nothing.
        if ( function_exists( 'get_registered_meta_keys' ) ) {
            $registered = get_registered_meta_keys( 'post', 'post' );
            if ( isset( $registered['td_post_video'] ) && ! empty( $registered['td_post_video']['show_in_rest'] ) ) {
                return;
            }
        }

        // Register/override meta key to ensure REST support is enabled.
        register_post_meta( 'post', 'td_post_video', array(
            ...
            'show_in_rest' => array(
                'schema' => array(
                    ...
                    // Keep permissive for forward compatibility with additional keys.
                    'additionalProperties' => true,
                ),
            ),

            // Only authenticated users who can edit posts can read/write via REST.
            'auth_callback' => static function () {
                return current_user_can( 'edit_posts' );
            },

            // Sanitize stored value; accept JSON string fallback if passed by clients.
            'sanitize_callback' => static function ( $value ) {
                ...
                if ( isset( $value['td_video'] ) ) {
                    $value['td_video'] = esc_url_raw( $value['td_video'] );
                }
                return $value;
            },
        ) );
    ...
    add_action( 'rest_api_init', 'td_register_featured_video_meta_rest' );
}

/**
 * Trigger video thumbnail generation after REST has saved td_post_video meta.
 * Supports forcing thumbnail replacement via td_post_video.td_force_thumb.
 */
add_action( 'rest_after_insert_post', function ( $post, $request, $creating ) {

    if ( ! class_exists( 'td_util' ) || td_util::get_option( 'tds_rest_featured_video' ) !== 'enabled' ) {
        return;
    }

    if ( ! class_exists( 'td_video_support' ) ) {
        return;
    }

    $meta = $request->get_param( 'meta' );
    if ( empty( $meta['td_post_video']['td_video'] ) ) {
        return;
    }

    $force_replace = ! empty( $meta['td_post_video']['td_force_thumb'] );

    td_video_support::rest_after_insert_generate_video_thumb( $post->ID, $force_replace );
