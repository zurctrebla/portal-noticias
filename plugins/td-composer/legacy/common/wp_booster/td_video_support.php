...
class td_video_support {
    ...
    private static $client_access_token = '716452503430928|2c98f1e896aae9f6e1699da31a9f0013'; // tagDiv Theme App
	...
	static function render_video($videoUrl, $controls = 'yes', $autoplay = '', $loop = '', $is_ajax = false) {
		...
		switch (self::detect_video_service($videoUrl)) {
			case 'youtube':
				...
				$video_time_param = self::get_youtube_time_param($videoUrl);
				$video_thumb_url = self::get_thumb_url( $videoUrl );

                $tds_video_lazy = td_util::get_option('tds_video_lazy');
                if ( ! empty($tds_video_lazy) && wp_is_mobile() && ! $is_ajax && ! td_util::is_amp()) {

                    isset( $video_info[$video_id]['title']) ? $value_alt = $video_info[$video_id]['title'] : $value_alt="";
		...
		height: 0;border-top: 7px solid transparent;border-bottom: 7px solid transparent;border-left: 10px solid #fff;\'></span></span></a>"
                    ...
                    if ( td_util::is_amp() ) {
                        ...
                        $buffy .= '
                            <div class="wpb_video_wrapper">
                                <iframe class="td-youtube-player" width="600" height="560" referrerpolicy="strict-origin-when-cross-origin" src="' . 'https://www.youtube.com/embed/' . urlencode_deep( self::get_youtube_id( $videoUrl ) ) . '?enablejsapi=1&feature=oembed&wmode=opaque&vq=hd720&' . $autoplayParam . '&' . $controlsParam . '&' . $loopParam . self::get_youtube_time_param( $videoUrl ) . '" frameborder="0" allowfullscreen="" allow="autoplay"></iframe>                            
                            </div>';
                    }
			    }
            ...
            case 'self-hosted':
                ...
                if ( $autoplay != '' && !( tdc_state::is_live_editor_ajax() || tdc_state::is_live_editor_iframe() ) ) {
                    $autoplayParam = 'autoplay muted playsinline';
                }
                if ( $loop != '' ) {
                    $loopParam = 'loop';
                }
    ...
    /**
     * Re-run the existing video thumbnail generation after REST has saved meta.
     */
    public static function rest_after_insert_generate_video_thumb( $post_id, $force_replace = false ) {

        // Load WP admin media stack needed by media_sideload_image().
        ...
        // If forced, remove existing featured image so the original method can generate a new one.
        if ( $force_replace && has_post_thumbnail( $post_id ) ) {
            delete_post_thumbnail( $post_id );
        }

        // Default behavior: never overwrite an existing featured image.
        ...
        // Allow retry when the guard matches and there is still no thumbnail.
        if ( ! empty( $td_post_video['td_video'] ) && ! empty( $td_last_set_video ) && $td_last_set_video === $td_post_video['td_video'] ) {
            delete_post_meta( $post_id, 'td_last_set_video' );
        }

        $prev = self::$on_save_post_did_action;
        self::$on_save_post_did_action = false;

        self::on_save_post_get_video_thumb( $post_id );

        self::$on_save_post_did_action = $prev;
    }
	...
	/**
	 * processes wp embedded youtube videos
	 */
	static function embed_oembed_html_process_embed_html( $html, $url, $attr, $post_ID ){

		/*
		 * add <div class="youtube-embed"> wrapper & youtube video id data for youtube videos
		 */
		...
		if ( strpos( $url, 'youtube.com' ) !== false ) {
			...
			// add the enablejsapi parameter
			$html = str_replace( '?feature=oembed', '?feature=oembed&enablejsapi=1', $html );

			// add a wrap to the html markup
			$new_html = '<div class="youtube-embed" data-video_id="' . $video_id . '">';
			$new_html .= $html;
			$new_html .= '</div>';

			// return the modified html markup
			return $new_html;

		}

		// return the unmodified HTML markup for non-YouTube URLs
		return $html;
    ...
    private static function get_youtube_id($videoUrl) {
        ...
        if (parse_url($videoUrl, PHP_URL_QUERY)) {
