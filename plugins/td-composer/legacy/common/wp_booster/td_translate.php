...
function __td( $td_string, $td_domain = '' ) {
    ...
    if ( !empty($td_translation_map_user[$td_string]) ) {   // return the user translation
        $translated = stripslashes($td_translation_map_user[$td_string]);
    } elseif ( !empty($td_translation_map[$td_string]) ) { // return the default translation or from MO file
        $translated = $td_translation_map[$td_string];
    } else {
        $translated = $td_string; // no translation detected - return the string
    }

    return preg_replace( '/%\s*(\d+)\s*\$\s*s/', '%$1$s', $translated );
