	...
	function _render_block_param($instance, $param, $show = false) {
		...
		switch ($param['type']) {
			...
			case 'textfield':
				...
				?>
				<p>
					<label for="<?php echo esc_attr($this->get_field_id($param['param_name'])) ?>">
                        <?php printf('%1$s', $param['heading']) ?>
                    </label>
					<input class="widefat" id="<?php echo esc_attr($field_id) ?>" name="<?php echo esc_attr($this->get_field_name($param['param_name'])) ?>" type="text" value="<?php echo esc_attr($instance[$param['param_name']]) ?>"/>
			...
			case 'custom':
				...
				?>
				<div>
                    <label><?php printf('%1$s', $param['heading']) ?></label>
                    <div class="td-wpa-info" style="margin-top: 0 !important;">
                        <?php echo html_entity_decode($param['value']); ?>
                    </div>
				</div>
				<?php
				break;
