            ...
            if (!is_null($this->post_thumb_id)) {
                ...
                if ( is_single() ) {
                    if ( $show_td_modal_image != 'no_modal' ) {
                        ...
                        $image_html .= '<img width="' . $featured_image_info['width'] . '" height="' . $featured_image_info['height'] . '" class="entry-thumb td-modal-image" src="' . $featured_image_info['src'] . '"' . $srcset_sizes . ' alt="' . esc_attr($featured_image_info['alt']) . '" title="' . esc_attr($featured_image_info['title']) . '"/>';
                        $image_html .= '</a>';
                    } else { //no_modal
                        $image_html = '<img width="' . $featured_image_info['width'] . '" height="' . $featured_image_info['height'] . '" class="entry-thumb" src="' . $featured_image_info['src'] . '"' .  $srcset_sizes . ' alt="' . esc_attr($featured_image_info['alt']) . '" title="' . esc_attr($featured_image_info['title']) . '"/>';
                    }
                } else {
                    //on blog index page
        ...
        */
        ...
        if (!empty($td_post_theme_settings['smart_list_template'])) {
            ...
            if (class_exists($td_smart_list_class)) {
                ...
                // there was an error?
                td_util::error(__FILE__, 'Missing smart list: ' . $td_smart_list_class . '. Did you disabled a ' . $brand . ' plugin?');
            }
        }
        /*  ----------------------------------------------------------------------------
...
//                        $section_parts = preg_split('/(<p.*>)/U', $content_part_value, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
    ...
    function get_item_scope() {
        ...
        if ($this->is_review && is_single() && td_util::get_option('tds_enable_products_schema_on_reviews') == '') {
    ...
    function get_item_scope_meta() {
        ...
        if ($this->is_review && td_util::get_option('tds_enable_products_schema_on_reviews') == '') {
    ...
    function get_author_box($author_id = '') {
        ...
        if (empty($hideAuthor)) {
            ...
            $buffy .= get_avatar( get_the_author_meta('email', $author_id), '96','', get_the_author_meta('display_name', $author_id) );
            $buffy .= '</a>';
