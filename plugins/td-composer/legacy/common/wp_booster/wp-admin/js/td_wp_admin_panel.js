...
function td_upload_image_font(id_upload_field) {
    ...
    jQuery(button).on( 'click', function() {
        ...
        window.send_to_editor = function(html) {
            ...
            var option = jQuery('#' + id_upload_field).attr('name'),
                match = option.match(/\[([^\]]+)\]/),
                option_name = match ? match[1] : null,
                img_link = jQuery('img', html).attr('src'),
                img_width = jQuery('img', html).attr('width'),
                img_height = jQuery('img', html).attr('height');

            if(typeof img_link == 'undefined') {
                var $html = jQuery(html);
                    img_link = $html.attr('src');
                    img_width = $html.attr('width');
                    img_height = $html.attr('height');
            }
            if(typeof img_link == 'undefined') {
            ...
            } else if (currentUploadField.hasClass('td_upload_field_link_image')) {

                jQuery("input[name='td_option[" + option_name + "_width]']").val(img_width);
                jQuery("input[name='td_option[" + option_name + "_height]']").val(img_height);
            }
        ...
        tdConfirm.showModal( 'Delete Image',
            ...
            function( $this ) {
                ...
                var controlId = $this.data('control-id'),
                    option = jQuery('#' + controlId).attr('name'),
                    match = option.match(/\[([^\]]+)\]/),
                    option_name = match ? match[1] : null;
                ...
                jQuery("input[name='td_option[" + option_name + "_width]']").val('');
                jQuery("input[name='td_option[" + option_name + "_height]']").val('');
 ...
 */
function td_panel_navigate(data_panel_id) {
