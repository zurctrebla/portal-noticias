                ...
                <div class="td-container-wrap">
                    ...
                    <div class="td-panel-main-header">
                        ...
                        <div class="td-panel-header-desc">
                            <span class="td-panel-header-name"><?php echo '<!-- panel header name -->' . td_util::get_wl_val('tds_wl_theme_name', $all_theme_panels_list[$panel_spot_id]['title']) ?></span>
        ...
        foreach ( td_global::$all_theme_panels_list[$td_current_panel_spot_id]['panels'] as $panel_id => $panel_array ) {
            ...
            if ( isset($panel_array['file']) and strpos($panel_array['file'], $td_ajax_calling_file) !== false ) {
                ...
                if ( $panel_array['type'] == 'in_theme' ) {
                    ...
                    $tdc_legacy_file_path = TDC_PATH_LEGACY . '/includes/panel/views/ajax_boxes/' . sanitize_file_name( $td_ajax_calling_file_id )  . '/' . sanitize_file_name( $td_ajax_box_id ) . '.php';

                    if ( file_exists( $tdc_legacy_file_path ) ) {

                        load_template( $tdc_legacy_file_path );

                    } else {

                        $tdc_legacy_common_file_path = TDC_PATH_LEGACY_COMMON . '/wp_booster/wp-admin/panel/views/ajax_boxes/' . sanitize_file_name( $td_ajax_calling_file_id ) . '/' . sanitize_file_name( $td_ajax_box_id ) . '.php';
                        require_once( $tdc_legacy_common_file_path );
                    ...
                    $ajax_box_plugin_path = $folder_path . '/' . sanitize_file_name( $td_ajax_calling_file_id ) . '/' . sanitize_file_name( $td_ajax_box_id );
