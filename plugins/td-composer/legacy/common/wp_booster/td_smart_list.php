...
abstract class td_smart_list {
	...
	/*
	 * !!!! The next _wp_link_page function should be moved to td_util class, this being a custom helper function.
...
//                $input_regex = str_replace(')', '\)', $input_regex);
    ...
    private function log_loop_complete() {
        if ($this->log === true) {
            echo '<!-- step complete -->' . $this->last_token_id . ' ' . $this->last_log_function . ' -- token: ' . $this->last_log_token . "\n";
