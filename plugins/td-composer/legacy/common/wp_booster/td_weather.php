...
//	);
	...
	static function render_generic($atts, $block_uid, $template = 'block_template')
	{
		...
		if (!td_util::tdc_is_live_editor_iframe() && !td_util::tdc_is_live_editor_ajax()) {
            ...
            if( TD_THEME_NAME == "Newspaper" ) {
                td_resources_load::render_script( TDC_SCRIPTS_URL . '/tdDatei18n.js' . TDC_SCRIPTS_VER, 'tdDatei18n-js', '', 'footer');
                td_resources_load::render_script( TDC_SCRIPTS_URL . '/tdAnimationSprite.js' . TDC_SCRIPTS_VER, 'tdAnimationSprite-js', '', 'footer');
                td_resources_load::render_script( TDC_SCRIPTS_URL . '/tdWeather.js' . TDC_SCRIPTS_VER, 'tdWeather-js', '', 'footer');
            }
			ob_start();
			?>
			<script>
	...
	private static function render_weather_section($param, $weather_data, $current_unit, $current_speed_label, $show_tooltip = false, $tooltip_text = '') {
		switch ($param) {
			case 'precipitation':
				$icon  = 'td-icons-rain';
				$label = 'Precipitation';
				$class = 'td-w-today-precipitation';
				$value = isset($weather_data['today_precipitation']) ? $weather_data['today_precipitation'] : 0;
				$value .= '%';
				break;
			case 'wind':
				$icon  = 'td-icons-wind';
				$label = 'Wind Speed';
				$class = 'td-w-today-wind-speed';
				$value = $weather_data['today_wind_speed'][$current_unit] . $current_speed_label;
				break;
			case 'clouds':
				$icon  = 'td-icons-cloud';
				$label = 'Cloud Cover';
				$class = 'td-w-today-clouds';
				$value = $weather_data['today_clouds'] . '%';
				break;
			default: // humidity
				$icon  = 'td-icons-drop';
				$label = 'Humidity';
				$class = 'td-w-today-humidity';
				$value = $weather_data['today_humidity'] . '%';
				break;
		}

		$title_attr = '';
		if ( $show_tooltip ) {
			$title      = $tooltip_text !== '' ? $tooltip_text : $label;
			$title_attr = ' title="' . esc_attr($title) . '"';
		}

		return '<i class="' . esc_attr($icon) . '"' . $title_attr . '></i>' .
		       '<span class="td-weather-parameter ' . esc_attr($class) . '"' . $title_attr . '>' . esc_html($value) . '</span>';
	}
	...
	private static function render_block_template($atts, $weather_data, $current_temp_label, $current_speed_label, $block_uid)
	{
		...
		<?php
			$w_param_1 = !empty($atts['w_param_1']) ? $atts['w_param_1'] : 'humidity';
			$w_param_2 = !empty($atts['w_param_2']) ? $atts['w_param_2'] : 'wind';
			$w_param_3 = !empty($atts['w_param_3']) ? $atts['w_param_3'] : 'clouds';

			$show_tooltips  = !empty($atts['w_tooltips']);
			$tooltip_text_1 = !empty($atts['w_tooltip_1_text']) ? $atts['w_tooltip_1_text'] : '';
			$tooltip_text_2 = !empty($atts['w_tooltip_2_text']) ? $atts['w_tooltip_2_text'] : '';
			$tooltip_text_3 = !empty($atts['w_tooltip_3_text']) ? $atts['w_tooltip_3_text'] : '';
		?>
		<div class="td-weather-info-wrap">
			<div class="td-weather-information">
				<div class="td-weather-section-1">
					<?php echo self::render_weather_section($w_param_1, $weather_data, $current_unit, $current_speed_label, $show_tooltips, $tooltip_text_1); ?>
				</div>
				<div class="td-weather-section-2">
					<?php echo self::render_weather_section($w_param_2, $weather_data, $current_unit, $current_speed_label, $show_tooltips, $tooltip_text_2); ?>
				</div>
				<div class="td-weather-section-3">
					<?php echo self::render_weather_section($w_param_3, $weather_data, $current_unit, $current_speed_label, $show_tooltips, $tooltip_text_3); ?>
				</div>
			</div>
	...
	private static function owm_get_today_data($atts, &$weather_data) {
		...
		if (isset($api_response['wind']['speed'])) {
			...
			$weather_data['today_wind_speed'][1] = self::mps_to_mph($api_response['wind']['speed']);
		}
	...
	private static function owm_get_five_days_data($atts, &$weather_data)
	{
		...
		if (!empty($api_response['list']) and is_array($api_response['list'])) {
			$tmp_temps = array();
			$max_pop   = 0;
			$cutoff_ts = time() + 86400; // next 24 hours

			foreach ($api_response['list'] as $index => $day_forecast) {

				if ( !empty($day_forecast['dt']) && isset($day_forecast['pop']) && $day_forecast['dt'] <= $cutoff_ts ) {
					$pop_pct = (int) round($day_forecast['pop'] * 100);
					if ($pop_pct > $max_pop) {
						$max_pop = $pop_pct;
					}
				}
			...
			$weather_data['today_precipitation'] = $max_pop;
			return true;
		}
