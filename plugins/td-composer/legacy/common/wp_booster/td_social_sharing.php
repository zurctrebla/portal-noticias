...
class td_social_sharing {
	...
	static function get_general_css($show = false) {
		...
		if ( td_util::is_mobile_theme() || 'Newsmag' === TD_THEME_NAME ) {
		    return;
        }
    ...
    private static function get_service_share_info($service_id, $post_id = null ) {
        ...
        //is not a post, check if it is a term
        if ( $post === null ) {
        ...
        switch ( $service_id ) {
            ...
            case 'twitter':
                ...
                return array(
                    'url' => 'https://x.com/intent/post?text=' . htmlspecialchars(urlencode(html_entity_decode($page_title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&url=' . urlencode($page_permalink) . $twitter_via_param,
                    'title' => 'X'
                );
                break;
