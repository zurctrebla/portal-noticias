...
class td_js {
        ...
        if (!empty($td_js_status_time)) {
            ...
            if ($td_js_status == 2) {
                ...
                if ( !tdc_check_domain::init()->is_domain_active() ) {
                    add_action('admin_notices', array($this, 'td_js_licence_inactive_on_domain_admin_notice'), 12);
                }
    ...
    /**
     * License inactive on domain admin notice.
     */
    function td_js_licence_inactive_on_domain_admin_notice() {
        if ( $this->check_if_is_our_page() === true || !current_user_can('publish_pages') ) {
            return;
        }
            ...
            <div class="about-wrap td-wp-admin-header">
                <div class="td-wp-admin-top">
                    ...
                    <div class="td-wp-admin-theme">
                        <h1>Your license of <?php echo td_util::get_wl_val('tds_wl_theme_name', TD_THEME_NAME) ?> Theme is <b style="color: red;">not assigned on this website!</b></h1>
                        <p>
                            Please go to the <strong>My License</strong> management panel and assign your <?php echo td_util::get_wl_val('tds_wl_theme_name', TD_THEME_NAME) ?> key to enjoy the full benefits of the theme. If you encounter any issues, don't hesitate to contact us at <a href="mailto:contact@tagdiv.com">contact@tagdiv.com</a>.
                        </p>
                        <p>
                            <?php echo '<a class="td-wp-admin-button" href="' . admin_url('admin.php?page=td_licence_panel') . '">Assign now</a>'?>
                        </p>
                    </div>
                </div>
            </div>
    ...
    function td_js_msg() {
        if ( $this->check_if_is_our_page() === true || td_api_features::is_enabled('require_activation') === false || !current_user_can('publish_pages') ) {
            return;
        }
