...
class tdc_guttenberg {
    ...
    public function tdc_add_admin_switch_button() {
        if ( !tdc_check_domain::init()->is_domain_active() || !$this->is_gutenberg_editor_active || !$this->show_on_this_post_type ) {
            return;
        }
        ...
        <script id="tdc-gutenberg-button-switch" type="text/html">
            <div class="tdc-panel-button">
                ...
                <div class="tdc-panel-link">
                    <a id="tdc-switch-button" href="' . tdc_util::get_edit_link( $this->post_id ) . '">' . td_util::get_wl_val('tds_wl_brand', 'TagDiv') . ' Composer</a>
                </div>
            </div>
        </script>';
    }
    ...
    public function __construct() {
		...
		// td yoast analyzer script
	    // @note works on pages, and it's used to render page content(theme shortcodes) to be analyzed by yoast's analyzer
	    add_action( 'admin_enqueue_scripts', function() {

		    if ( $this->is_gutenberg_editor_active && $this->show_on_this_post_type ) {

			    if ( TDC_DEPLOY_MODE == 'deploy' ) {
				    wp_enqueue_script( 'js_files_for_gutenberg_yoast_analyzer', TDC_URL . '/assets/js/js_files_for_gutenberg_yoast_analyzer.min.js', array( 'jquery', 'lodash' ), TD_COMPOSER, true );
			    } else {
				    wp_enqueue_script( 'tdcYoastAnalyzer', TDC_URL . '/assets/js/tdcYoastAnalyzer.js', array( 'jquery', 'lodash' ), TD_COMPOSER, true );
			    }
