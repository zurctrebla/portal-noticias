...
//	));
    ...
    register_rest_route( $namespace, '/td_cloud_proxy/', array(
        ...
        'callback' => function($request) {
            ...
            // Update the transient holding whether the registered theme is active on
            // current domain.
            if ( $cloud_end_point == 'templates/check' ) {
                $api_response_body = json_decode($api_response['body'], true);
                tdc_check_domain::set_transient(
                    (
...
//		$reply_html = ob_get_clean();
        ...
        // If the element is inside a row, we need to set the 'in_row' global flag
        // and also determine whether it's an Ai Builder section.
        $parent_row_attributes = $request->get_param('parentRowAttributes');
        $parent_row_attributes = !empty($parent_row_attributes) ? json_decode($parent_row_attributes, true) : array();
        $is_aib_active = defined('TD_AIB_PLUGIN_NAME');
        $is_parent_row_aib_section = false;

        if ( !empty($parent_row_attributes) ) {
            td_global::set_in_row(true);
            $is_parent_row_aib_section = isset($parent_row_attributes['aib_is_section']) && $parent_row_attributes['aib_is_section'] == 'yes';

            if ( $is_aib_active && $is_parent_row_aib_section ) {
                td_global::set_td_aib_section((object) array(
                    'id'    => !empty($parent_row_attributes['aib_section_id']) ? $parent_row_attributes['aib_section_id'] : '',
                    'type'  => !empty($parent_row_attributes['aib_section_type']) ? $parent_row_attributes['aib_section_type'] : '',
                    'title' => !empty($parent_row_attributes['aib_section_title']) ? $parent_row_attributes['aib_section_title'] : '',
                    'style' => !empty($parent_row_attributes['aib_section_style']) ? $parent_row_attributes['aib_section_style'] : TagDiv\AiBuilder\Styles::get_active_global_style()
                ));
            }
        }
        ...
        // If necessary, reset the 'in_row' global flag and the Ai Builder section global variable.
        if ( !empty($parent_row_attributes) ) {
            td_global::set_in_row(false);

            if ( $is_aib_active && $is_parent_row_aib_section ) {
                td_global::set_td_aib_section(null);
            }
        }
    ...
    static function on_ajax_save_parts( WP_REST_Request $request ) {
        ...
        // Recursive sanitizer for all "name" keys
        $sanitize_names_recursive = function( $data ) use ( &$sanitize_names_recursive ) {
            foreach ( $data as $key => $value ) {

                // If key is "name", sanitize it
                ...
                if ( is_array( $value ) ) {
                    $data[$key] = $sanitize_names_recursive( $value );
                }
            }
            return $data;
        };
        ...
        if ( ! isset( $action ) || 'tdc_ajax_save_parts' !== $action ) {
            ...
            if ( isset( $tdc_savings ) ) {
                ...
                // Sanitize all the name keys
                $tdc_savings = $sanitize_names_recursive( $tdc_savings );
