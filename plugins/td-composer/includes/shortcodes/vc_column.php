...
class vc_column extends tdc_composer_block {
	...
	function render($atts, $content = null) {
		...
		if ( !empty( $is_sticky_param ) ) {
            ...
            // Indexes: 0 = phone, 1 = portrait, 2 = landscape, 3 = desktop (all)
            $sticky_enabled_on = array(false, false, false, false);

            if ( td_util::is_base64( $is_sticky_param ) ) {

                $decoded = json_decode( base64_decode( $is_sticky_param ) );

                if ( ! empty( $decoded ) && is_object( $decoded ) ) {

                    if ( property_exists( $decoded, 'phone' ) ) {
                        $sticky_enabled_on[0] = ( $decoded->phone === 'yes' );
                    }

                    if ( property_exists( $decoded, 'portrait' ) ) {
                        $sticky_enabled_on[1] = ( $decoded->portrait === 'yes' );
                    }

                    if ( property_exists( $decoded, 'landscape' ) ) {
                        $sticky_enabled_on[2] = ( $decoded->landscape === 'yes' );
                    }

                    if ( property_exists( $decoded, 'all' ) ) {
                        $sticky_enabled_on[3] = ( $decoded->all === 'yes' );
                    }
                }

            } else {
                $on = ( $is_sticky_param === 'yes' || $is_sticky_param === '1' || $is_sticky_param === 1 );
                $sticky_enabled_on = array( $on, $on, $on, $on );
            }
        ...
        if( $hide_for_user_type != '' ) {
            if( !( td_util::tdc_is_live_editor_ajax() || td_util::tdc_is_live_editor_iframe() ) &&
                ...
                $column_class .= ' tdc-restr-display-none';
            }
        } else {
