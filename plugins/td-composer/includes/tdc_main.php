...
require_once('tdc_check_domain.php');
...
function tdc_on_admin_bar_menu() {
	...
	if (
	    ...
	    if ( empty($is_mobile_template)) {
            ...
            if ( 'Newspaper' === TD_THEME_NAME  && td_global::is_tdb_registered() && tdc_check_domain::init()->is_domain_active() ) {
	            $wp_admin_bar->add_menu( array(
...
function tdc_on_load_font_icon() {
    ...
    // add icon fonts used on tabbed content pages
    $tc_pages_icon_fonts = tdc_util::get_tabbed_content_pages_icon_fonts_ids( $post_id );
    if ( !empty( $tc_pages_icon_fonts ) && is_array( $tc_pages_icon_fonts ) ) {
        foreach ( $tc_pages_icon_fonts as $font_id => $font_settings ) {
            if ( ! isset( $icon_fonts[ $font_id ] ) ) {
                $icon_fonts[ $font_id ] = $font_settings;
            }
        }
...
if (!empty($td_action)) {
	...
	switch ($td_action) {
		...
		case 'tdc_edit':
			...
			// Capability check - the user must be allowed to edit the requested post.
            if ( !current_user_can( 'edit_pages' ) ) {
                wp_die( __( 'Sorry, you are not allowed to edit this item.' ) );
            }

            // Nonce check: prevent CSRF content injection via tdc_content POST param
