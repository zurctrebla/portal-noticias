...
if(window.tdbVue&&"undefined"!==typeof window.tdbVue.router){var eventHub=new CloudLibraryVue;CloudLibraryVue.component("image-links",{template:'\n            <a :class="classes" :href="link">{{text}}</a>\n        ',props:{classes:[],link:"",text:""}});window.eventHub.$on("refresh-global-colors",function(){jQuery.ajax({url:td_ajax_url,type:"post",data:{action:"tdc_wm_global_colors_update",td_magic_token:tdc_wm_global_colors_nonce},success:function(a){a=JSON.parse(a);if(void 0!==a.error)console.log("%c"+
a.error,"color: #dc2121c7");else{window.tdc_wm_global_colors=a.tdc_wm_global_colors;var b=":root {";Object.entries(a.tdc_wm_global_colors).forEach(function(a){b+="--"+a[0].replace(/_/g,"-")+":"+a[1].color+";"});b+="}";a=jQuery("style#tdb-global-colors");a.length&&a.html(b);a=window.document.getElementById("tdc-live-iframe");null!==a&&(a=jQuery("style#tdb-global-colors",a.contentWindow.document),a.length&&a.html(b))}},error:function(a,b,c){console.log("%c"+c,"color: #dc2121c7")}})});window.eventHub.$on("refresh-custom-svg-icons",
function(){jQuery.ajax({url:td_ajax_url,type:"post",data:{action:"tdc_wm_custom_svg_icons_get_all",td_magic_token:tdc_wm_custom_svg_icons_nonce},success:function(a){a=JSON.parse(a);void 0!==a.error?console.log("%c"+a.error,"color: #dc2121c7"):(window.tdc_wm_custom_svg_icons=a.tdc_wm_custom_svg_icons,Object.entries(a.tdc_wm_custom_svg_icons).forEach(function(a){jQuery("#tdc-icon-selector").each(function(){jQuery(this).find('.td-icon-svg[data-font_class="td-icon-'+a[0]+'"]').length||jQuery(this).trigger("tdc_wm_custom_svg_icons_update",
["new",{id:a[0],name:a[1].name,code:a[1].code}])})}))},error:function(a,b,c){console.log("%c"+c,"color: #dc2121c7")}})});window.eventHub.$on("refresh-global-fonts",function(){jQuery.ajax({url:td_ajax_url,type:"post",data:{action:"tdc_wm_global_fonts_get_all",td_magic_token:tdc_wm_global_fonts_nonce},success:function(a){a=JSON.parse(a);if(void 0!==a.error)console.log("%c"+a.error,"color: #dc2121c7");else{window.tdc_wm_global_fonts=a.tdc_wm_global_fonts;var b=Object.entries(window.tdcAdminSettings.mappedFontFamily),
c=[];Object.entries(a.tdc_wm_global_fonts).forEach(function(a){var d=a[0];-1===b.findIndex(function(a){a=$jscomp.makeIterator(a);a.next();return a.next().value===d+"_global"})&&c.push([a[1].name,d+"_global"])});if(c.length){var d=b.findIndex(function(a){return"-- Font Stacks --"===$jscomp.makeIterator(a).next().value});window.tdcAdminSettings.mappedFontFamily=Object.fromEntries([].concat($jscomp.arrayFromIterable(b.slice(0,d)),$jscomp.arrayFromIterable(c),[b[d]],$jscomp.arrayFromIterable(b.slice(d+
1))))}Object.entries(window.tdcAdminSettings.mappedShortcodes).forEach(function(a){a=$jscomp.makeIterator(a);a.next();a=a.next().value;Object.entries(a.params).forEach(function(a){a=$jscomp.makeIterator(a);a.next();a=a.next().value;"dropdown-responsive"===a.type&&"tdc-font-dropdown tdc-font-family"===a.class&&(a.value=window.tdcAdminSettings.mappedFontFamily)})});var f=":root {";Object.entries(a.tdc_wm_global_fonts).forEach(function(a){f+="--"+a[0]+":"+a[1].font_family+";"});f+="}";a=jQuery("style#tdb-global-fonts");
a.length&&a.html(f);a=window.document.getElementById("tdc-live-iframe");null!==a&&(a=jQuery("style#tdb-global-fonts",a.contentWindow.document),a.length&&a.html(f))}},error:function(a,b,c){console.log("%c"+c,"color: #dc2121c7")}})});new CloudLibraryVue({el:"#tdb-modal",router:window.tdbVue.router})}window.tdbVueManager&&"undefined"!==typeof window.tdbVueManager.router&&new CloudLibraryVue({el:"#tdb-modal-website-manager",router:window.tdbVueManager.router});(function(){jQuery('#menu-posts-tdb_templates a[href$="post-new.php?post_type=tdb_templates"], #wp-admin-bar-new-tdb_templates a[href$="post-new.php?post_type=tdb_templates"]').hide()})();
...
(function(a){a().wpColorPicker&&a("input.tdb-tax-term-color-picker").wpColorPicker();a(document).off("click","button.tdb_upload_image_button");a(document).on("click","button.tdb_upload_image_button",function(b){var c=this;b.preventDefault();b.stopPropagation();var d=void 0;"undefined"!==typeof wp&&wp.media&&wp.media.editor&&(d||(d=wp.media.frames.select_image=wp.media({title:"Choose an Image",button:{text:"Use Image"},multiple:!1}),d.on("select",function(){var b=d.state().get("selection").first().toJSON();
if(""!==a.trim(b.id)){var e="undefined"===typeof b.sizes.thumbnail?b.sizes.full.url:b.sizes.thumbnail.url;a(c).prev().val(b.id);a(c).closest(".meta-image-field-wrapper").find("img").attr("src",e);a(c).next().show()}}),d.on("open",function(){var b=d.state().get("selection"),e=a(c).prev().val();e=wp.media.attachment(e);e.fetch();b.add(e?[e]:[])})),d.open())});a(document).on("click","button.tdb_remove_image_button",function(b){b.preventDefault();b.stopPropagation();b=a(this).closest(".meta-image-field-wrapper").find("img").data("placeholder");
a(this).closest(".meta-image-field-wrapper").find("img").attr("src",b);a(this).prev().prev().val("");a(this).hide();return!1})})(jQuery);var tdbCloudTemplates={};(function(a,b){tdbCloudTemplates={init:function(){window.tdbCloudTemplatesVue&&"undefined"!==typeof window.tdbCloudTemplatesVue.router&&a("#tdb-cloud-templates").length&&new CloudLibraryVue({el:"#tdb-cloud-templates",router:window.tdbCloudTemplatesVue.router})}};tdbCloudTemplates.init()})(jQuery,_);
