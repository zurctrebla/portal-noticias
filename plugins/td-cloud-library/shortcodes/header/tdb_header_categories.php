...
class tdb_header_categories extends td_block {
    ...
    function render( $atts, $content = null ) {
        ...
        if ( $raw_tdicon === '' ) {
            ...
            if ( $is_base64 ) {
                ...
                $svg_safe = tdb_header_categories::sanitize_inline_svg( $svg );
                if ( $svg_safe !== '' ) {
                    $toggle_icon_html = '<span class="tdb-head-cat-toggle tdb-head-cat-toggle-svg" ' . $tdicon_data . '>' . $svg_safe . '</span>';
                } else {
                    $toggle_icon_html = '<i class="tdb-head-cat-toggle td-icon-mobile"></i>';
                }
            } else {
                // icon class(es): validate each class token against allowed prefixes
                $icon_class_raw   = trim( $raw_tdicon );
                $tokens           = preg_split( '/\s+/', $icon_class_raw );
                $allowed_prefixes = apply_filters( 'tdb_header_categories_icon_prefixes', array(
                    'td-icon',        // theme icons
                    'tdc-font-',      // theme font icon packs (e.g., tdc-font-fa, tdc-font-typcn)
                    'fa', 'fas', 'far', 'fal', 'fad', 'fab',  // Font Awesome variants
                    'mdi', 'ti', 'la', 'ri', 'bi', 'icon'     // common icon packs
                ) );

                $safe_tokens = array();
                foreach ( $tokens as $t ) {
                    if ( $t === '' ) { continue; }
                    // allow only simple class characters
                ...
                if ( ! empty( $safe_tokens ) ) {
                    $toggle_icon_html = '<i class="tdb-head-cat-toggle ' . esc_attr( implode( ' ', $safe_tokens ) ) . '"></i>';
                } else {
                    $toggle_icon_html = '<i class="tdb-head-cat-toggle td-icon-mobile"></i>';
                }
            }
        ...
        $buffy .= '<div class="' . esc_attr( $this->get_block_classes() ) . ' tdb-header-align" ' . $this->get_block_html_atts() . '>';
        ...
        foreach ( $categories as $category ) {
            ...
            if ( $show_bg != '' ) {
                ...
                if ( $cat_bg_img != '' ) {
                    $cat_bg_img_html = ' style="background-image:url(' . esc_url( $cat_bg_img ) . ')"';
                }
            }
            ...
            if ( $show_overlay != '' ) {
                ...
                $cat_overlay = '<span class="tdb-head-cat-overlay" style="' . esc_attr( 'background:linear-gradient(-30deg,' . $cat_overlay_1 . ',' . $cat_overlay_2 . ')' ) . '"></span>';
            }

            $buffy .= '<div class="tdb-head-cat-item">';
            $buffy .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '"' . $cat_bg_img_html . '>';
            $buffy .= $cat_overlay;
            $buffy .= '<span class="tdb-head-cat-txt">' . esc_html( $category->name ) . '</span>';
            $buffy .= '</a>';
            $buffy .= '</div>';
        }
        $buffy .= '</div>';
    ...
    /**
     * Sanitize inline SVG markup to prevent XSS.
     */
        ...
        if ( function_exists( 'wp_kses' ) ) {
            $allowed = array(
                'svg'   => array(
                    ...
                    'role' => true, 'aria-hidden' => true, 'focusable' => true, 'fill' => true,
                ),
