    ...
    public function get_custom_css() {
        ...
        $raw_css =
            "<style>
                    ...
                    .td_block_social_counter.td-social-style9 .td_social_twitch .td-sp {
                    ...
                    @media (min-width: 768px) and (max-width: 1018px) {
                     ...
                     .td_block_social_counter.td-social-style10 .td_social_twitch .td-sp {
                        background-position: -4px -766px;
                    }
                    @media (min-width: 768px) and (max-width: 1018px) {
                        .td_block_social_counter.td-social-style10 .td_social_twitch .td-sp {
                    ...
                    /* @btn_color */
                    div.$unique_block_class .td_social_type .td_social_button a {
                        color: @btn_color;
                    }
                    /* @btn_color_h */
	...
	function render( $atts, $content = null ) {
        ...
        foreach ( td_social_icons::$td_social_icons_array as $td_social_id => $td_social_name ) {
            ...
            if ( !empty($atts[$td_social_id]) ) {
                ...
                // -----------------------------
                // SAFE NUMBER FORMAT + FORMAT CHECK
                // -----------------------------
                $api_display = '';
                if ( isset( $social_network_meta['api'] ) && $social_network_meta['api'] !== '' && $social_network_meta['api'] !== null ) {

                    // original raw string
                    $api_raw = trim( (string) $social_network_meta['api'] );

                    // allow "12 345" and "12,345" by stripping spaces/commas
                    $normalized = str_replace( array( ' ', ',' ), '', $api_raw );

                    if ( is_numeric( $normalized ) ) {
                        // valid numeric -> safe to format
                        $api_display = number_format( (float) $normalized );
                    } else {
                        // invalid format -> mark flag, show raw string, but DO NOT call number_format()
                        $invalid_social_format = true;
                        $api_display           = $api_raw;
                    }
                }
                ...
                $socialBuffy .= '<div class="td_social_type td-pb-margin-side td_social_' . $td_social_id . '">';
                    $socialBuffy .= '<div class="td-social-box">';
                        ...
                        $socialBuffy .= '<span class="td_social_info td_social_info_counter">' . $api_display . '</span>';
                        $socialBuffy .= '<span class="td_social_info td_social_info_name">' . $social_network_meta['text'] . '</span>';
                        $socialBuffy .= '<span class="td_social_button">';
                            $socialBuffy .= '<a href="' . $social_network_meta['url'] . '" ' . $td_target . $td_social_rel. ' >' . $social_network_meta['button'] . '</a>';
                        $socialBuffy .= '</span>';
                    $socialBuffy .= '</div>';
                $socialBuffy .= '</div>';
        ...
        //$buffy .= '<pre style="color: grey;">$atts: ';
        //$buffy .= print_r( $atts, true );;
        //$buffy .= '</pre>';

        if ( ( $socialEmpty || $invalid_social_format ) && is_user_logged_in() && current_user_can( 'switch_themes' ) ) {
            ...
            if ( $socialEmpty ) {
                ...
                $buffy .= td_util::get_block_error(
                    'Social counter',
                    "Configure this block/widget's settings to get socials"
                );

            } elseif ( $invalid_social_format ) {
                $buffy .= td_util::get_block_error(
                    'Social counter',
                    'One or more manual counter values have an incorrect format. Please use only numbers (e.g. 12345).'
                );
            }
    ...
    function get_social_network_meta( $service_id, $user_id, &$td_social_api, $access_token = '' ) {
	    ...
	    switch ($service_id) {
            ...
            case 'tiktok':
                return array(
                    'button' => __td('Follow'),
                    'url' => "https://www.tiktok.com/" . strtok($user_id,  '/'),
                    'text' => __td('Followers'),
                    'api' => $td_social_api->get_social_counter($service_id, $user_id, $access_token),
                );
